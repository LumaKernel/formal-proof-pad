...
Hilbert系の形式的体系は、採用する公理スキーマの選択によって多くのバリエーションがある。特に否定 (`¬`) の扱いは体系ごとに大きく異なる。
...
| 流儀                  | 否定公理                                          | 性格                                      | 代表的な体系            |
| --------------------- | ------------------------------------------------- | ----------------------------------------- | ----------------------- |
| 対偶 (Contraposition) | $(¬φ → ¬ψ) → (ψ → φ)$                             | 「¬φが¬ψを含意するなら、ψがφを含意する」  | Łukasiewicz             |
| 背理法 (Reductio)     | $(¬φ → ¬ψ) → ((¬φ → ψ) → φ)$                      | 「¬φから矛盾が導かれるなら、φが成り立つ」 | Mendelson               |
| 二重否定除去 + 対偶   | $(φ → ψ) → (¬ψ → ¬φ)$, $¬¬φ → φ$, $φ → ¬¬φ$       | 対偶+DNE+DNIの3公理                       | Frege                   |
| 爆発 + 場合分け       | $φ → (¬φ → ψ)$, $(φ → ψ) → ((¬φ → ψ) → ψ)$        | ex falsoと排中律的推論の2公理             | Hilbert-Ackermann       |
| DNE + 自己矛盾 + 対偶 | $¬¬φ → φ$, $(φ → ¬φ) → ¬φ$, $(φ → ¬ψ) → (ψ → ¬φ)$ | 3つの否定公理                             | Russell-Whitehead       |
| ⊥経由のDNE            | $((φ → ⊥) → ⊥) → φ$                               | falsum経由の二重否定除去                  | Church                  |
| Peirce則 + 爆発       | $((φ → ψ) → φ) → φ$, $⊥ → φ$                      | 古典推論と爆発を分離                      | Tarski-Bernays-Wajsberg |
...
- $φ ∨ ¬φ$（排中律, LEM）
- $((φ → ψ) → φ) → φ$（Peirceの法則）
- $(¬φ → ¬ψ) → (ψ → φ)$（対偶, Łukasiewicz L3）
- $(¬φ → ¬ψ) → ((¬φ → ψ) → φ)$（背理法, Mendelson M3）
...
| ---- | ------------------------------------- | ----------------- |
| M1   | $φ → (ψ → φ)$                         | K公理（L1と同一） |
| M2   | $(φ → (ψ → χ)) → ((φ → ψ) → (φ → χ))$ | S公理（L2と同一） |
| M3   | $(¬φ → ¬ψ) → ((¬φ → ψ) → φ)$          | 背理法            |

**L3とM3の違い:**

- **L3（対偶）:** $(¬φ → ¬ψ) → (ψ → φ)$ — 「¬φが¬ψを含意するなら、ψがφを含意する」
- **M3（背理法）:** $(¬φ → ¬ψ) → ((¬φ → ψ) → φ)$ — 「¬φが¬ψとψの両方を含意するなら（矛盾するなら）、φが成り立つ」
...
| ---- | ------------------------------------- | ------------------------------------- |
| F1   | $φ → (ψ → φ)$                         | 弱化                                  |
| F2   | $(χ → (ψ → φ)) → ((χ → ψ) → (χ → φ))$ | 含意の分配                            |
| F3   | $(φ → (ψ → χ)) → (ψ → (φ → χ))$       | 前件の交換（冗長、F1+F2から導出可能） |
| F4   | $(φ → ψ) → (¬ψ → ¬φ)$                 | 対偶                                  |
| F5   | $¬¬φ → φ$                             | 二重否定除去                          |
| F6   | $φ → ¬¬φ$                             | 二重否定導入                          |
...
| ---- | ------------------------------- | -------------------------- |
| H1   | $φ → (ψ → φ)$                   | 弱化                       |
| H2   | $(φ → (ψ → χ)) → (ψ → (φ → χ))$ | 前件の交換                 |
| H3   | $(ψ → χ) → ((φ → ψ) → (φ → χ))$ | 合成                       |
| H4   | $φ → (¬φ → ψ)$                  | 爆発（ex falso quodlibet） |
| H5   | $(φ → ψ) → ((¬φ → ψ) → ψ)$      | 場合分け                   |
...
| ---- | ------------------------------------- | ------------------- |
| C1   | $φ → (ψ → φ)$                         | 弱化                |
| C2   | $(φ → (ψ → χ)) → ((φ → ψ) → (φ → χ))$ | 含意の分配          |
| C3   | $((φ → ⊥) → ⊥) → φ$                   | ⊥経由の二重否定除去 |

**特徴:** ¬φを φ→⊥ と展開すれば、C3は $¬¬φ → φ$ に等しい。否定を独立した結合子ではなく、⊥への含意として統一的に扱う。

**出典:** Church, "Introduction to Mathematical Logic" (1956)

### 3.6 Tarski-Bernays-Wajsberg体系（⊥ベース）
...
| 記号 | 式                              | 名称         |
| ---- | ------------------------------- | ------------ |
| TBW1 | $(φ → ψ) → ((ψ → χ) → (φ → χ))$ | 仮言三段論法 |
| TBW2 | $φ → (ψ → φ)$                   | 弱化         |
| TBW3 | $((φ → ψ) → φ) → φ$             | Peirceの法則 |
| TBW4 | $⊥ → φ$                         | 爆発         |
...
| 記号 | 式                              | 名称                                    |
| ---- | ------------------------------- | --------------------------------------- |
| La1  | $(φ → ψ) → ((ψ → χ) → (φ → χ))$ | 仮言三段論法                            |
| La2  | $(¬φ → φ) → φ$                  | Claviusの法則（consequentia mirabilis） |
| La3  | $φ → (¬φ → ψ)$                  | 爆発                                    |

**特徴:** L1-L3とは全く異なるスタイル。La2「¬φからφが導かれるなら、φは無条件に成り立つ」は背理法の一形態。
...
否定は $¬φ := φ → ⊥$ と定義されるが、$⊥ → φ$（爆発）は**導出不可能**。

**意味:** 矛盾から任意の命題が導かれることはない。「矛盾しているからといって何でも言える」わけではない。

弱い対偶 $(φ → ψ) → (¬ψ → ¬φ)$ と二重否定導入 $φ → ¬¬φ$ は証明可能だが、二重否定除去 $¬¬φ → φ$ と爆発 $¬φ → (φ → ψ)$ は証明不可能。
...
**特殊な性質:** 選言性（disjunction property）— $\vdash φ ∨ ψ$ ならば $\vdash φ$ または $\vdash ψ$
...
ML: 正の命題計算 + ⊥（公理なし）
IPC: ML + 爆発 (⊥ → φ)
CPC: IPC + DNE (¬¬φ → φ)  [or LEM, Peirce, etc.]
```

## 5. 連言・選言の扱い: 原始 vs 定義

### 5.1 定義として導入する場合（→, ¬ のみ原始的）
...
**欠点:** 直観主義論理では成り立たない定義がある（$φ ∨ ψ := ¬φ → ψ$ は古典論理でのみ等価）。

### 5.2 原始的な結合子として追加する場合

∧, ∨ を原始的結合子として追加し、それぞれに導入/除去の公理を追加する（セクション4.2参照）。
...
| アプローチ | 原始結合子 | 否定          | 利点                                       |
| ---------- | ---------- | ------------- | ------------------------------------------ |
| ¬ベース    | →, ¬       | 原始的        | 直感的。AST上でNegationが独立したノード    |
| ⊥ベース    | →, ⊥       | $¬φ := φ → ⊥$ | 統一的。最小/直観主義/古典の切り替えが容易 |
...
- ⊥ベース: Negation を `Implication(φ, Bottom)` に展開して扱う

この二層構造により、ユーザーは常に ¬ を使って式を入力でき、体系の内部表現は設定に従う。
...
| ------------------------ | ---------- | ------------------------------------- | -------- | ---------- |
| `lukasiewicz-classical`  | →, ¬       | L1, L2, L3                            | MP       | 古典       |
| `mendelson-classical`    | →, ¬       | M1, M2, M3                            | MP       | 古典       |
| `church-classical`       | →, ⊥       | C1, C2, C3                            | MP       | 古典       |
| `intuitionistic-minimal` | →, ∧, ∨, ⊥ | K, S, ∧-intro/elim, ∨-intro/elim, ⊥→φ | MP       | 直観主義   |
| `minimal-logic`          | →, ∧, ∨, ⊥ | K, S, ∧-intro/elim, ∨-intro/elim      | MP       | 最小       |
...
| 記号 | 式                          | 名称             | 制約                              |
| ---- | --------------------------- | ---------------- | --------------------------------- |
| A4   | $∀x. φ(x) → φ(t)$           | 全称例化         | $t$ が $x$ に対して自由に代入可能 |
| A5   | $∀x. (φ → ψ) → (φ → ∀x. ψ)$ | 全称と含意の分配 | $x$ が $φ$ に自由出現しない       |

### 7.2 追加される推論規則

| 規則       | 形式                    | 制約                               |
| ---------- | ----------------------- | ---------------------------------- |
| 汎化 (Gen) | $φ$ から $∀x. φ$ を導出 | $x$ が未解消の仮定に自由出現しない |

### 7.3 存在量化子

$∃x. φ(x) := ¬∀x. ¬φ(x)$

**注意:** 直観主義論理では $∃$ を独立した結合子として扱い、専用の公理を与える必要がある。
...
2. **LogicSystem設定で公理体系を切り替え可能にする**
   ...
   - オプション: Mendelson, Church(⊥ベース), 直観主義, 最小論理

3. **表面表現と内部表現の翻訳層を設ける**
   - ∧, ∨, ↔ は体系の原始結合子でなければ定義に基づいて展開
   - ¬ は ⊥ベース体系では φ→⊥ に展開
   - 展開は証明検証時に透過的に適用される
...
- Church, A. "Introduction to Mathematical Logic" (1956) — ⊥ベース体系
