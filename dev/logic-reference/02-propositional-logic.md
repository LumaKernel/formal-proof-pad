...
本プロジェクトではŁukasiewicz（ウカシェヴィチ）による最小限の公理体系を採用する。含意 `→` と否定 `¬` を基本結合子とし、3つの公理スキーマからなる。
...
**AST対応:** `Implication(φ, Implication(ψ, φ))`
...
**AST対応:** `Implication(Implication(φ, Implication(ψ, χ)), Implication(Implication(φ, ψ), Implication(φ, χ)))`
...
**AST対応:** `Implication(Implication(Negation(φ), Negation(ψ)), Implication(ψ, φ))`
...
| 種類         | ラベル | 前提数 | 説明                                                       |
| ------------ | ------ | ------ | ---------------------------------------------------------- |
| Modus Ponens | MP     | 2      | 前提1: $\alpha$, 前提2: $\alpha \to \beta$ → 結論: $\beta$ |
...
```
  A2[ψ:=φ→φ, χ:=φ]                       A1[ψ:=φ→φ]
─────────────────────────────────────    ──────────────────────
(φ→((φ→φ)→φ))→((φ→(φ→φ))→(φ→φ))        φ→((φ→φ)→φ)          A1[ψ:=φ]
──────────────────────────────────────────────────────── (MP)   ──────────────
        (φ→(φ→φ))→(φ→φ)                                        φ→(φ→φ)
        ───────────────────────────────────────────────── (MP)
                            φ→φ
...
| 5        | $(\lnot \lnot \lnot \varphi \to \lnot \lnot \varphi) \to (\lnot \varphi \to \lnot \lnot \lnot \varphi)$                                                       | A3: $\varphi := \lnot \lnot \varphi$, $\psi := \lnot \varphi$ → 結果を変形                   |
