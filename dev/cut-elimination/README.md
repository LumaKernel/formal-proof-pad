...
| #   | ドキュメント                                                  | 概要                                                                                                                           |
| --- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| 01  | [シーケント計算の基礎](./01-sequent-calculus.md)              | シーケント（Γ ⊢ Δ）の定義、LK の推論規則（構造規則・論理規則・切断規則・公理）、LJ との違い                                    |
...
| 04  | [具体例によるカット除去の実演](./04-examples.md)              | 命題論理（→）のカット除去、量化子（∀）のカット除去、入れ子カットの段階的除去をステップバイステップで追跡                       |
...
| 日本語                     | 英語                               | 説明                                                              |
| -------------------------- | ---------------------------------- | ----------------------------------------------------------------- |
| シーケント                 | sequent                            | Γ ⊢ Δ の形の判断。左辺は仮定、右辺は結論                          |
| シーケント計算             | sequent calculus                   | Gentzen が導入した証明体系。LK（古典）/ LJ（直観主義）            |
| カット規則 / 切断規則      | cut rule                           | Γ ⊢ Δ, A と A, Σ ⊢ Π から Γ, Σ ⊢ Δ, Π を導く規則                  |
...
| 公理 / 恒等式              | identity / axiom                   | A ⊢ A の形の初期シーケント                                        |
