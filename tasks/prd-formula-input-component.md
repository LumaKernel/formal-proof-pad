...
- [ ] `<TermDisplay term={ast} />` コンポーネント（Unicode版）を作成
- [ ] `<TermKaTeX term={ast} />` コンポーネント（KaTeX版）を作成
- [ ] 二項演算子（+, −, ×, ÷, ^）、関数適用、定数、変数を正しく表示
- [ ] Storybookストーリーを追加
- [ ] 型チェック/lintが通る
...
- [ ] `<FormulaInput value={text} onChange={handler} />` コンポーネントを作成
- [ ] テキスト入力でリアルタイムにパースを実行
- [ ] パース成功時: 入力欄の下にUnicode表示でプレビュー
- [ ] パースエラー時: エラー位置（行:列）とメッセージをインライン表示
- [ ] エラー位置の該当テキストにアンダーラインハイライト
- [ ] `onParsed` コールバックでパース成功時のFormulaASTを親に通知
- [ ] debounce付き（入力中の過剰なパースを防止）
- [ ] Storybookストーリー（正常入力、エラー入力、リアルタイム編集）を追加
- [ ] play関数で入力→パース→表示のフローを検証
- [ ] 型チェック/lintが通る
...
- [ ] Storybookストーリー（モード切替、エラー時の挙動）を追加
- [ ] play関数でモード切替フローを検証
- [ ] 型チェック/lintが通る
