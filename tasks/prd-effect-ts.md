...
### フェーズ1: 基盤整備・既存Effect活用の深化 (logic-core)
...
- [ ] **ET-001: logic-core エラー型を `Data.TaggedError` に統一**
  ...
  **対象ファイルと変換内容:**
  ...
  1. **`src/lib/logic-core/inferenceRule.ts`** — `RuleApplicationError` (10バリアント)
     ...
     export type RuleApplicationError =
       ...
       | { readonly _tag: "PremiseMismatch"; readonly expected: Formula; readonly actual: Formula }
       | ...;
  ...
  **影響を受けるファイル（importの更新が必要）:**
  - `src/lib/logic-core/inferenceRule.test.ts` — Result型の検証を `Either.match` / `Either.isRight` に
  - `src/lib/logic-core/unification.test.ts` — 同上
  - `src/lib/proof-pad/mpApplicationLogic.ts` — `RuleApplicationResult` の使い方変更
  - `src/lib/proof-pad/genApplicationLogic.ts` — 同上

  **手順:**
  1. context7 で `Data.TaggedError` の最新APIを確認
  2. `unification.ts` のエラー型を変換（小さいので先にやる）
  3. `unification.test.ts` を更新、テスト通過を確認
  4. `inferenceRule.ts` のエラー型を変換
  5. `inferenceRule.test.ts` を更新、テスト通過を確認
  6. 依存先（proof-pad）の型エラーを修正（ET-004で本格対応するが、コンパイル通るようにはしておく）
  7. `npm run typecheck && npm run lint && npm run test` 通過を確認

  **受け入れ基準:** すべてのlogic-coreエラー型が `Data.TaggedError` ベースになり、全テスト通過
...
- [ ] **ET-002: Schema.decode/encode によるAST検証・シリアライゼーション**
  ...
  **対象ファイル:**
  ...
  - `src/lib/logic-core/serialization.test.ts` (新規作成)
  ...
  **提供する関数:**
  ...
  // JSON unknown → Formula (バリデーション付き)
  export const decodeFormula: (input: unknown) => Either<Formula, Schema.ParseError> = ...
  // Formula → JSON-safe object
  export const encodeFormula: (formula: Formula) => unknown = ...
  // 同様に Term 用も
  export const decodeTerm: (input: unknown) => Either<Term, Schema.ParseError> = ...
  export const encodeTerm: (term: Term) => unknown = ...
  ```

  **テスト計画:**
  - 各ASTノード種別のencode/decodeラウンドトリップ
  - 不正なJSON入力（`_tag` が不正、フィールド欠落等）に対するエラー
  - 再帰的な構造（ネストした Formula）のラウンドトリップ
  - `Schema.suspend` を使った再帰型が正しく decode されることの確認

  **手順:**
  1. context7 で `Schema.decodeUnknown`, `Schema.encodeUnknown` の最新APIを確認
  2. `serialization.ts` を作成
  3. テストを書いてからの実装（TDD）
  4. `index.ts` にre-export追加
  5. テスト通過を確認

  **受け入れ基準:** Formula/Term のJSON入出力が Schema ベースで型安全に動作し、ラウンドトリップテスト通過
...
- [ ] **ET-003: unification の内部を `Effect.gen` で整理**
  ...
  **対象ファイル:**
  ...
  - `src/lib/logic-core/unification.test.ts`
  ...
  **注意:** ET-001 完了後に着手すること（エラー型が `Data.TaggedError` である前提）

  **手順:**
  1. `unifyFormulas` と内部ヘルパーを `Effect.gen` に変換
  2. `unifyFormulasSync` 互換ラッパーを提供
  3. テストは `Effect.runSync(Effect.either(...))` でラップ
  4. 既存テストがすべて通過することを確認
  5. 呼び出し元（`inferenceRule.ts` 等）が壊れていないことを確認

  **受け入れ基準:** unification内部がEffect化され、エラー伝搬が `yield*` ベースになる。全テスト通過
...
- [ ] **ET-005: MP/Gen適用ロジックを `Effect.gen` パイプラインに**
