...
- [x] shadcn-ui wayに以下を変えていって
  ...
  - [x] アップバーのタイトルブランド
  - [x] トップページのタブ
  - [x] リファレンスの検索欄、各検索ボタン、検索結果のドキュメントカード
  - [x] 各タブのボタンやアイコンボタン
  - [x] 各種フォーム(ノートやクエスト作成)
  - [x] パッド内の各種アクションをするための起点、ボタンなど
- [x] かっこいいfaviconを作って置き換えて。アップバーのタイトルブランドもテーマにちなんだ形にかっこいいものに（ターンスタイル⊢モチーフのSVGファビコン＋カスタムブランドアイコン）
- [x] リファレンスの上のほうに、入門者はこちらから、これを学ぶならここから、のように章だてのガイドの起点となるものを並べよう。（guideカテゴリのエントリを番号付きカードで表示するガイドセクション追加）
- [x] マークダウンの中でも Unicode の φ などが使われてる箇所があるが、すべてTeXに統一しよう。
  - InlineMarkdownに$...$のKaTeXインライン数式レンダリングを追加
...
- [x] http://localhost:13006/?path=/story/pages-workspace--quest-complete-prop-01
  - 公理axiom-onlyステップからbuildModelAnswerWorkspaceで初期化し、play関数でMP2回適用→ゴール達成の完全インタラクション実装
  - [x] また、ヒルベルト流以外はアプリ側は色々と不完全だろう。証明の流れのサイドパネルも出ないし、MP適用などのボタンは出たままになる。
    - [x] まずは prd-non-hilbert.mdで個別体系ごとに、どのような対応が必要かを網羅的にタスクリストにして進めよう — 作成済み。MP/GenはisHilbertStyleガード済みだがSubstitutionメニューは未ガード

- [x] http://localhost:13006/?path=/story/formulainput-formulaeditor--error-stays-in-edit-mode
  - [x] エラー時の文字色が背景色と同じで見えない — エラーハイライトオーバーレイの color: transparent を修正。FormulaInput/TermInput両方

## FormulaEditor統一タスク (2026-03-17)

- [x] 過去の以下のタスクが完了できていない
  - [x] MP適用などのモーダルの、論理式入力のところは、論理式入力のための共通UIをせっかく作ったのだから、それを利用したクリックから開始して、離れたらrenderされるやつを使うほうがよいだろう
  - [x] 基本すべての箇所において、 phi -> phi とかのままではなく、編集から離れたら render されるように共通化されるべき
  - AxiomPalette: unicodeDisplay文字列をFormulaDisplayに置換。EditableProofNode: パース失敗時フォールバックに数式フォントスタイル適用。GoalPanel: フォールバックにrole="math"とaria-label付与。EdgeParameterPopover は既にFormulaInput/TermInput使用済み。
    === 引用終了 ===
  ...
  - [x] 基本はrenderedが表示され、クリックで編集開始できるやつ (Nodeに使われてるダブルクリックのクリック版) を利用せよ。
        http://localhost:13006/?path=/story/formulainput-formulaeditor--with-parsed-callback を指している。
    - ProofWorkspace代入プロンプトのFormulaInput→FormulaEditor、TermInput→TermEditorに置換。クリックで編集開始、離れたらrendered表示に統一
...
- [x] ライブラリの充実（汎用2種+Hilbert2種=4テンプレート追加、計18テンプレート）
