...
- ただし、不完全に完了したもの、ゴールを完全に達成しきらなかったものは `tasks-failed/...` に移動する
...
**push 後は必ず CI の結果を確認すること。確認せずに次の作業に進んではならない。**
...
ローカルテストだけでは検出できない問題（環境差異、依存関係など）を早期発見するため。push しっぱなしで結果を見ないのは push していないのと同じ。
