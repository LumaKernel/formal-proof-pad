...
0. git commit / git pushが必要か確認する。
0. CIの結果を確認する。完了していなければ待機する。
0. `tasks/*.md` を読む。**`tasks/inserted-tasks.md` が存在し未完了タスクがあれば、他のタスクより優先して対処すること。**
0. `task-processing.md` に実行するタスクを書き写す。どのファイルから書き写したか明記する。既に存在すれば進捗を確認し、続きから開始する。
...
0. **【開始前確認】** テスト・カバレッジのベースラインを記録する（下記「テスト・カバレッジ確認」参照）
0. その単一のタスクを実装する。**ステップ4で立てたテスト・ストーリー計画に沿って進める。**
0. **【完了前確認】** テスト・カバレッジを再確認し、低下していないことを確認
0. 品質チェックを実行する（typecheck, lint, test など、プロジェクトで必要なもの）
   Playwright MCPを利用して、http://localhost:13006/ にアクセスし、スクリーンショットも screenshots.local/ に保存しながら確認作業を行う。
0. 再利用可能なパターンを発見した場合は CLAUDE.md ファイルを更新する（下記参照）
0. すべての変更をコミット。メッセージは**日本語**で: `feat: [領域] - [ストーリーの内容を日本語で簡潔に]`
0. git pushしてCIの結果を確認する。CIは必ず待機し、失敗していれば成功するまで修正を繰り返す。
0. `tasks/*.md` を更新する（下記「タスクファイルのアーカイブ」参照）。
0. `task-processing.md` の中身を空にする。完全に終わっていなければ、申し送り事項を追記して終了する。
0. `progress.txt` に進捗を追記する（カバレッジの変化も記録）
