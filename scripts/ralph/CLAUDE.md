...
6. その単一のユーザーストーリーを実装する
7. **【完了前確認】** テスト・カバレッジを再確認し、低下していないことを確認
8. 品質チェックを実行する（typecheck, lint, test など、プロジェクトで必要なもの）
9. 再利用可能なパターンを発見した場合は AGENTS.md ファイルを更新する（下記参照）
10. チェックが通ったら、すべての変更をコミット。メッセージは**日本語**で: `feat: [Story ID] - [ストーリーの内容を日本語で簡潔に]`
11. PRD を更新して、完了したストーリーの `passes: true` を設定する
12. `progress.txt` に進捗を追記する（カバレッジの変化も記録）
