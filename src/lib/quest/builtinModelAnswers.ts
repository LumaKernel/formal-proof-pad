...
/**
 * prop-49: 対偶公理のA1持ち上げ φ → ((¬ψ → ¬χ) → (χ → ψ))
 *
 * A3をA1で前提の下に持ち上げる。3ステップ。
 *
 * 証明:
 * 0. A3[φ/ψ, ψ/χ]: (¬ψ → ¬χ) → (χ → ψ)
 * 1. A1: ((¬ψ→¬χ)→(χ→ψ)) → (φ → ((¬ψ→¬χ)→(χ→ψ)))
 * 2. MP(0, 1): φ → ((¬ψ→¬χ)→(χ→ψ))
 */
const prop49A3LiftedInstance: ModelAnswer = {
  ...
  steps: [
    ...
    {
      _tag: "axiom",
      formulaText: "(~psi -> ~chi) -> (chi -> psi)",
    },
    // 1. A1: ((¬ψ→¬χ)→(χ→ψ)) → (φ → ((¬ψ→¬χ)→(χ→ψ)))
    {
      _tag: "axiom",
      formulaText:
        "((~psi -> ~chi) -> (chi -> psi)) -> (phi -> ((~psi -> ~chi) -> (chi -> psi)))",
    },
    // 2. MP(0, 1): φ → ((¬ψ→¬χ)→(χ→ψ))
    { _tag: "mp", leftIndex: 0, rightIndex: 1 },
  ],
 ...
 *
 * 証明戦略（81ステップ）:
 * 1. DNI for φ: φ → ¬¬φ [37 steps]
 * 2. CONJ-DEF forward: (φ ∧ ¬φ) → ¬(φ → ¬¬φ) [1 step]
 * 3. DNE for (φ ∧ ¬φ): ¬¬(φ ∧ ¬φ) → (φ ∧ ¬φ) [35 steps]
 * 4. Compose: ¬¬(φ ∧ ¬φ) → ¬(φ → ¬¬φ) [5 steps]
 * 5. A3 + MP: (φ → ¬¬φ) → ¬(φ ∧ ¬φ) → MP with DNI → ¬(φ ∧ ¬φ) [3 steps]
 */
const prop30LNC: ModelAnswer = {
  ...
  steps: [
    ...
    // 36. MP(34, 35): φ → ¬¬φ [DNI]
    { _tag: "mp", leftIndex: 34, rightIndex: 35 },
    // --- CONJ-DEF forward: (φ ∧ ¬φ) → ¬(φ → ¬¬φ) ---
    // 37.
    {
      _tag: "axiom",
      formulaText: "(phi /\\ ~phi) -> ~(phi -> ~~phi)",
    },
    // --- DNE for (φ ∧ ¬φ): prop-17 inline with φ := (φ ∧ ¬φ), 35 steps ---
    // 38.
    {
      _tag: "axiom",
      formulaText: "~~(phi /\\ ~phi) -> (~(phi /\\ ~phi) -> ~~(phi /\\ ~phi))",
    ...
    // 42. MP(39, 41)
    { _tag: "mp", leftIndex: 39, rightIndex: 41 },
    // 43. MP(42, 40)
    { _tag: "mp", leftIndex: 42, rightIndex: 40 },
    // 44. MP(38, 43)
    { _tag: "mp", leftIndex: 38, rightIndex: 43 },
    // 45.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 47.
    {
      _tag: "axiom",
      formulaText:
        "(~(phi /\\ ~phi) -> ((~~(~(phi /\\ ~phi) -> (phi /\\ ~phi)) -> ~(phi /\\ ~phi)) -> ((phi /\\ ~phi) -> ~(~(phi /\\ ~phi) -> (phi /\\ ~phi))))) -> ((~(phi /\\ ~phi) -> (~~(~(phi /\\ ~phi) -> (phi /\\ ~phi)) -> ~(phi /\\ ~phi))) -> (~(phi /\\ ~phi) -> ((phi /\\ ~phi) -> ~(~(phi /\\ ~phi) -> (phi /\\ ~phi)))))",
    },
    // 48.
    {
      _tag: "axiom",
    ...
    // 49. MP(45, 48)
    { _tag: "mp", leftIndex: 45, rightIndex: 48 },
    // 50. MP(49, 47)
    { _tag: "mp", leftIndex: 49, rightIndex: 47 },
    // 51. MP(46, 50)
    { _tag: "mp", leftIndex: 46, rightIndex: 50 },
    // 52.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 55.
    {
      _tag: "axiom",
      formulaText:
        "((~(phi /\\ ~phi) -> (phi /\\ ~phi)) -> ((~(phi /\\ ~phi) -> ~(~(phi /\\ ~phi) -> (phi /\\ ~phi))) -> ((~(phi /\\ ~phi) -> (phi /\\ ~phi)) -> (phi /\\ ~phi)))) -> (((~(phi /\\ ~phi) -> (phi /\\ ~phi)) -> (~(phi /\\ ~phi) -> ~(~(phi /\\ ~phi) -> (phi /\\ ~phi)))) -> ((~(phi /\\ ~phi) -> (phi /\\ ~phi)) -> ((~(phi /\\ ~phi) -> (phi /\\ ~phi)) -> (phi /\\ ~phi))))",
    },
    // 56.
    {
      _tag: "axiom",
    ...
    // 57. MP(54, 56)
    { _tag: "mp", leftIndex: 54, rightIndex: 56 },
    // 58. MP(57, 55)
    ...
    // 64. MP(63, 62)
    { _tag: "mp", leftIndex: 63, rightIndex: 62 },
    // 65.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 66. MP(65, 64)
    { _tag: "mp", leftIndex: 65, rightIndex: 64 },
    // 67. MP(66, 61)
    { _tag: "mp", leftIndex: 66, rightIndex: 61 },
    // 68.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 70. MP(67, 69)
    { _tag: "mp", leftIndex: 67, rightIndex: 69 },
    // 71. MP(70, 68)
    { _tag: "mp", leftIndex: 70, rightIndex: 68 },
    // 72. MP(44, 71): ¬¬(φ ∧ ¬φ) → (φ ∧ ¬φ) [DNE]
    { _tag: "mp", leftIndex: 44, rightIndex: 71 },
    // --- compose: ¬¬(φ ∧ ¬φ) → ¬(φ → ¬¬φ) ---
    // 73. A2
    {
      _tag: "axiom",
    ...
    // 75. MP(37, 74)
    { _tag: "mp", leftIndex: 37, rightIndex: 74 },
    // 76. MP(75, 73)
    { _tag: "mp", leftIndex: 75, rightIndex: 73 },
    // 77. MP(72, 76): ¬¬(φ ∧ ¬φ) → ¬(φ → ¬¬φ)
    { _tag: "mp", leftIndex: 72, rightIndex: 76 },
    // --- A3 + MP: ¬(φ ∧ ¬φ) ---
    // 78. A3[α:=¬(φ∧¬φ), β:=(φ→¬¬φ)]
    {
      _tag: "axiom",
    ...
    // 79. MP(77, 78): (φ → ¬¬φ) → ¬(φ ∧ ¬φ)
    { _tag: "mp", leftIndex: 77, rightIndex: 78 },
    // 80. MP(36, 79): ¬(φ ∧ ¬φ)
    { _tag: "mp", leftIndex: 36, rightIndex: 79 },
  ],
};
 ...
 *
 * 戦略: 219ステップ
 * 1. DNE for ψ (prop-17 inline, 35 steps): ¬¬ψ → ψ
 * 2. Inner compose (4 steps): (¬φ→¬¬ψ) → (¬φ→ψ)
 * 3. DNE for (¬φ→¬¬ψ) (prop-17 inline, 35 steps): ~~(¬φ→¬¬ψ) → (¬φ→¬¬ψ)
 * 4. DNI for (¬φ→ψ) (prop-25 inline, 37 steps): (¬φ→ψ) → ~~(¬φ→ψ)
 * 5. Triple compose (10 steps): ~~(¬φ→¬¬ψ) → ~~(¬φ→ψ)
 * 6. A3 + MP (2 steps): ¬(¬φ→ψ) → ¬(¬φ→¬¬ψ)
 * 7. DISJ-DEF bridge (85 steps): ¬(φ∨ψ) → ¬(¬φ→ψ)
 *    - DISJ-DEF backward + DNE(¬φ→ψ) + DNI(φ∨ψ) + compose + A3
 * 8. Final compose (11 steps): ¬(φ∨ψ) → (¬φ∧¬ψ)
 *    - compose with keyLemma + CONJ-DEF backward
 */
const prop24DeMorgan: ModelAnswer = {
  questId: "prop-24",
  steps: [
    // --- DNE for ψ: prop-17 inline, 35 steps ---
    // 0.
    { _tag: "axiom", formulaText: "~~psi -> (~psi -> ~~psi)" },
    // 1.
    { _tag: "axiom", formulaText: "(~psi -> ~~psi) -> (~psi -> psi)" },
    ...
    // 37. MP(34, 36)
    { _tag: "mp", leftIndex: 34, rightIndex: 36 },
    // 38. MP(37, 35)
    { _tag: "mp", leftIndex: 37, rightIndex: 35 },
    // --- DNE for (~φ→~~ψ): prop-17 inline, 35 steps ---
    // 39.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 73. MP(45, 72)
    { _tag: "mp", leftIndex: 45, rightIndex: 72 },
    // --- DNI for (~φ→ψ): prop-25 inline, 37 steps ---
    // 74.
    {
      _tag: "axiom",
      formulaText: "~~~(~phi -> psi) -> (~~(~phi -> psi) -> ~~~(~phi -> psi))",
    ...
    { _tag: "mp", leftIndex: 78, rightIndex: 76 },
    // 80. MP(74, 79)
    { _tag: "mp", leftIndex: 74, rightIndex: 79 },
    // 81.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 85. MP(81, 84)
    { _tag: "mp", leftIndex: 81, rightIndex: 84 },
    // 86. MP(85, 83)
    { _tag: "mp", leftIndex: 85, rightIndex: 83 },
    // 87. MP(82, 86)
    { _tag: "mp", leftIndex: 82, rightIndex: 86 },
    // 88.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 89. MP(87, 88)
    { _tag: "mp", leftIndex: 87, rightIndex: 88 },
    // 90.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 93. MP(90, 92)
    { _tag: "mp", leftIndex: 90, rightIndex: 92 },
    // 94. MP(93, 91)
    { _tag: "mp", leftIndex: 93, rightIndex: 91 },
    // 95. MP(89, 94)
    { _tag: "mp", leftIndex: 89, rightIndex: 94 },
    // 96.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 97. MP(95, 96)
    { _tag: "mp", leftIndex: 95, rightIndex: 96 },
    // 98.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 100. MP(99, 98)
    { _tag: "mp", leftIndex: 99, rightIndex: 98 },
    // 101.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 102. MP(101, 100)
    { _tag: "mp", leftIndex: 101, rightIndex: 100 },
    // 103. MP(102, 97)
    { _tag: "mp", leftIndex: 102, rightIndex: 97 },
    // 104.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 106. MP(103, 105)
    { _tag: "mp", leftIndex: 103, rightIndex: 105 },
    // 107. MP(106, 104)
    { _tag: "mp", leftIndex: 106, rightIndex: 104 },
    // 108. MP(80, 107)
    { _tag: "mp", leftIndex: 80, rightIndex: 107 },
    // 109.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 110. MP(108, 109)
    { _tag: "mp", leftIndex: 108, rightIndex: 109 },
    // --- triple compose: ~~(~φ→~~ψ) → ~~(~φ→ψ) ---
    // 111.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 113. MP(38, 112)
    { _tag: "mp", leftIndex: 38, rightIndex: 112 },
    // 114. MP(113, 111)
    { _tag: "mp", leftIndex: 113, rightIndex: 111 },
    // 115. MP(73, 114)
    { _tag: "mp", leftIndex: 73, rightIndex: 114 },
    // 116.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 118. MP(110, 117)
    { _tag: "mp", leftIndex: 110, rightIndex: 117 },
    // 119. MP(118, 116)
    { _tag: "mp", leftIndex: 118, rightIndex: 116 },
    // 120. MP(115, 119)
    { _tag: "mp", leftIndex: 115, rightIndex: 119 },
    // --- A3 + MP: ¬(~φ→ψ) → ¬(~φ→~~ψ) ---
    // 121.
    {
      _tag: "axiom",
    ...
    // 122. MP(120, 121)
    { _tag: "mp", leftIndex: 120, rightIndex: 121 },
    // --- DISJ-DEF bridge: ¬(φ∨ψ) → ¬(~φ→ψ) ---
    // 123.
    { _tag: "axiom", formulaText: "(~phi -> psi) -> (phi \\/ psi)" },
    // 124.
    {
    ...
    { _tag: "mp", leftIndex: 128, rightIndex: 126 },
    // 130. MP(124, 129)
    { _tag: "mp", leftIndex: 124, rightIndex: 129 },
    // 131.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 135. MP(131, 134)
    { _tag: "mp", leftIndex: 131, rightIndex: 134 },
    // 136. MP(135, 133)
    { _tag: "mp", leftIndex: 135, rightIndex: 133 },
    // 137. MP(132, 136)
    { _tag: "mp", leftIndex: 132, rightIndex: 136 },
    // 138.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 143. MP(140, 142)
    { _tag: "mp", leftIndex: 140, rightIndex: 142 },
    // 144. MP(143, 141)
    { _tag: "mp", leftIndex: 143, rightIndex: 141 },
    // 145. MP(139, 144)
    { _tag: "mp", leftIndex: 139, rightIndex: 144 },
    // 146.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 147. MP(145, 146)
    { _tag: "mp", leftIndex: 145, rightIndex: 146 },
    // 148.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 150. MP(149, 148)
    { _tag: "mp", leftIndex: 149, rightIndex: 148 },
    // 151.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 152. MP(151, 150)
    { _tag: "mp", leftIndex: 151, rightIndex: 150 },
    // 153. MP(152, 147)
    { _tag: "mp", leftIndex: 152, rightIndex: 147 },
    // 154.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 156. MP(153, 155)
    { _tag: "mp", leftIndex: 153, rightIndex: 155 },
    // 157. MP(156, 154)
    { _tag: "mp", leftIndex: 156, rightIndex: 154 },
    // 158. MP(130, 157)
    { _tag: "mp", leftIndex: 130, rightIndex: 157 },
    // 159.
    {
      _tag: "axiom",
      formulaText: "~~~(phi \\/ psi) -> (~~(phi \\/ psi) -> ~~~(phi \\/ psi))",
    ...
    // 163. MP(160, 162)
    { _tag: "mp", leftIndex: 160, rightIndex: 162 },
    // 164. MP(163, 161)
    { _tag: "mp", leftIndex: 163, rightIndex: 161 },
    // 165. MP(159, 164)
    { _tag: "mp", leftIndex: 159, rightIndex: 164 },
    // 166.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 170. MP(166, 169)
    { _tag: "mp", leftIndex: 166, rightIndex: 169 },
    // 171. MP(170, 168)
    { _tag: "mp", leftIndex: 170, rightIndex: 168 },
    // 172. MP(167, 171)
    { _tag: "mp", leftIndex: 167, rightIndex: 171 },
    // 173.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 174. MP(172, 173)
    { _tag: "mp", leftIndex: 172, rightIndex: 173 },
    // 175.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 178. MP(175, 177)
    { _tag: "mp", leftIndex: 175, rightIndex: 177 },
    // 179. MP(178, 176)
    { _tag: "mp", leftIndex: 178, rightIndex: 176 },
    // 180. MP(174, 179)
    { _tag: "mp", leftIndex: 174, rightIndex: 179 },
    // 181.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 182. MP(180, 181)
    { _tag: "mp", leftIndex: 180, rightIndex: 181 },
    // 183.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 185. MP(184, 183)
    { _tag: "mp", leftIndex: 184, rightIndex: 183 },
    // 186.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 187. MP(186, 185)
    { _tag: "mp", leftIndex: 186, rightIndex: 185 },
    // 188. MP(187, 182)
    { _tag: "mp", leftIndex: 187, rightIndex: 182 },
    // 189.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 191. MP(188, 190)
    { _tag: "mp", leftIndex: 188, rightIndex: 190 },
    // 192. MP(191, 189)
    { _tag: "mp", leftIndex: 191, rightIndex: 189 },
    // 193. MP(165, 192)
    { _tag: "mp", leftIndex: 165, rightIndex: 192 },
    // 194.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 195. MP(193, 194)
    { _tag: "mp", leftIndex: 193, rightIndex: 194 },
    // 196.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 198. MP(123, 197)
    { _tag: "mp", leftIndex: 123, rightIndex: 197 },
    // 199. MP(198, 196)
    { _tag: "mp", leftIndex: 198, rightIndex: 196 },
    // 200. MP(158, 199)
    { _tag: "mp", leftIndex: 158, rightIndex: 199 },
    // 201.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 203. MP(195, 202)
    { _tag: "mp", leftIndex: 195, rightIndex: 202 },
    // 204. MP(203, 201)
    { _tag: "mp", leftIndex: 203, rightIndex: 201 },
    // 205. MP(200, 204)
    { _tag: "mp", leftIndex: 200, rightIndex: 204 },
    // 206.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 207. MP(205, 206)
    { _tag: "mp", leftIndex: 205, rightIndex: 206 },
    // --- final compose: ¬(φ∨ψ) → (¬φ∧¬ψ) ---
    // 208.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 210. MP(122, 209)
    { _tag: "mp", leftIndex: 122, rightIndex: 209 },
    // 211. MP(210, 208)
    { _tag: "mp", leftIndex: 210, rightIndex: 208 },
    // 212. MP(207, 211)
    { _tag: "mp", leftIndex: 207, rightIndex: 211 },
    // 213.
    { _tag: "axiom", formulaText: "~(~phi -> ~~psi) -> (~phi /\\ ~psi)" },
    // 214.
    {
 ...
 *
 * 戦略: 249ステップ
 * 1. prop-29 inline with ψ:=χ (163 steps): (φ→χ) → ((¬φ→χ) → χ)  [TND]
 * 2. B-combinator theorem (7 steps): (ψ→χ) → ((¬φ→ψ) → (¬φ→χ))
 * 3. Composition (24 steps): ((¬φ→χ)→χ) → ((ψ→χ) → ((¬φ→ψ)→χ))
 * 4. B-comp with TND (5 steps): (φ→χ) → ((ψ→χ) → ((¬φ→ψ)→χ))
 * 5. C-combinator (20 steps): argument permutation ψ→χ / ¬φ→ψ
 * 6. B-comp (5 steps): (φ→χ) → ((¬φ→ψ) → ((ψ→χ)→χ))
 * 7. C-combinator (20 steps): argument permutation φ→χ / ¬φ→ψ
 * 8. DISJ-DEF bridge (6 steps): (φ∨ψ) → ((φ→χ) → ((ψ→χ)→χ))
 */
const prop32DisjElim: ModelAnswer = {
  ...
  steps: [
    ...
    {
      _tag: "axiom",
      formulaText: "(~~chi -> ~(~~chi -> ~chi)) -> ((~~chi -> ~chi) -> ~chi)",
    },
    {
      _tag: "axiom",
      formulaText:
    ...
    { _tag: "mp", leftIndex: 69, rightIndex: 70 },
    {
      _tag: "axiom",
      formulaText:
        "((~~phi -> (phi -> chi))) -> ((~~phi -> phi) -> (~~phi -> chi))",
    },
    {
      _tag: "axiom",
      formulaText: "((phi -> chi)) -> ((~~phi -> (phi -> chi)))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((phi -> chi) -> ((~~phi -> (phi -> chi)) -> ((~~phi -> phi) -> (~~phi -> chi)))) -> (((phi -> chi) -> (~~phi -> (phi -> chi))) -> ((phi -> chi) -> ((~~phi -> phi) -> (~~phi -> chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((~~phi -> (phi -> chi)) -> ((~~phi -> phi) -> (~~phi -> chi))) -> ((phi -> chi) -> ((~~phi -> (phi -> chi)) -> ((~~phi -> phi) -> (~~phi -> chi))))",
    },
    { _tag: "mp", leftIndex: 72, rightIndex: 75 },
    { _tag: "mp", leftIndex: 76, rightIndex: 74 },
    { _tag: "mp", leftIndex: 73, rightIndex: 77 },
    {
      _tag: "axiom",
      formulaText:
        "((phi -> chi) -> ((~~phi -> phi) -> (~~phi -> chi))) -> (((phi -> chi) -> (~~phi -> phi)) -> ((phi -> chi) -> (~~phi -> chi)))",
    },
    { _tag: "mp", leftIndex: 78, rightIndex: 79 },
    {
      _tag: "axiom",
      formulaText: "((~~phi -> phi)) -> ((phi -> chi) -> (~~phi -> phi))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((~~phi -> phi) -> (((phi -> chi) -> (~~phi -> phi)) -> ((phi -> chi) -> (~~phi -> chi)))) -> (((~~phi -> phi) -> ((phi -> chi) -> (~~phi -> phi))) -> ((~~phi -> phi) -> ((phi -> chi) -> (~~phi -> chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((phi -> chi) -> (~~phi -> phi)) -> ((phi -> chi) -> (~~phi -> chi))) -> ((~~phi -> phi) -> (((phi -> chi) -> (~~phi -> phi)) -> ((phi -> chi) -> (~~phi -> chi))))",
    },
    { _tag: "mp", leftIndex: 80, rightIndex: 83 },
    { _tag: "mp", leftIndex: 84, rightIndex: 82 },
    { _tag: "mp", leftIndex: 81, rightIndex: 85 },
    ...
    {
      _tag: "axiom",
      formulaText: "((chi -> ~~chi)) -> ((~~phi -> (chi -> ~~chi)))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((chi -> ~~chi) -> ((~~phi -> (chi -> ~~chi)) -> ((~~phi -> chi) -> (~~phi -> ~~chi)))) -> (((chi -> ~~chi) -> (~~phi -> (chi -> ~~chi))) -> ((chi -> ~~chi) -> ((~~phi -> chi) -> (~~phi -> ~~chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((~~phi -> (chi -> ~~chi)) -> ((~~phi -> chi) -> (~~phi -> ~~chi))) -> ((chi -> ~~chi) -> ((~~phi -> (chi -> ~~chi)) -> ((~~phi -> chi) -> (~~phi -> ~~chi))))",
    },
    { _tag: "mp", leftIndex: 88, rightIndex: 91 },
    { _tag: "mp", leftIndex: 92, rightIndex: 90 },
    { _tag: "mp", leftIndex: 89, rightIndex: 93 },
    { _tag: "mp", leftIndex: 71, rightIndex: 94 },
    {
      _tag: "axiom",
      formulaText:
        "((phi -> chi) -> ((~~phi -> chi) -> (~~phi -> ~~chi))) -> (((phi -> chi) -> (~~phi -> chi)) -> ((phi -> chi) -> (~~phi -> ~~chi)))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((~~phi -> chi) -> (~~phi -> ~~chi)) -> ((phi -> chi) -> ((~~phi -> chi) -> (~~phi -> ~~chi)))",
    },
    { _tag: "mp", leftIndex: 95, rightIndex: 97 },
    { _tag: "mp", leftIndex: 98, rightIndex: 96 },
    { _tag: "mp", leftIndex: 87, rightIndex: 99 },
    { _tag: "axiom", formulaText: "(~~phi -> ~~chi) -> (~chi -> ~phi)" },
    {
      _tag: "axiom",
      formulaText:
        "((phi -> chi) -> ((~~phi -> ~~chi) -> (~chi -> ~phi))) -> (((phi -> chi) -> (~~phi -> ~~chi)) -> ((phi -> chi) -> (~chi -> ~phi)))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((~~phi -> ~~chi) -> (~chi -> ~phi)) -> ((phi -> chi) -> ((~~phi -> ~~chi) -> (~chi -> ~phi)))",
    },
    { _tag: "mp", leftIndex: 101, rightIndex: 103 },
    { _tag: "mp", leftIndex: 104, rightIndex: 102 },
    { _tag: "mp", leftIndex: 100, rightIndex: 105 },
    {
      _tag: "axiom",
      formulaText:
        "((~chi -> (~phi -> chi))) -> ((~chi -> ~phi) -> (~chi -> chi))",
    },
    {
      _tag: "axiom",
      formulaText: "((~phi -> chi)) -> ((~chi -> (~phi -> chi)))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((~phi -> chi) -> ((~chi -> (~phi -> chi)) -> ((~chi -> ~phi) -> (~chi -> chi)))) -> (((~phi -> chi) -> (~chi -> (~phi -> chi))) -> ((~phi -> chi) -> ((~chi -> ~phi) -> (~chi -> chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((~chi -> (~phi -> chi)) -> ((~chi -> ~phi) -> (~chi -> chi))) -> ((~phi -> chi) -> ((~chi -> (~phi -> chi)) -> ((~chi -> ~phi) -> (~chi -> chi))))",
    },
    { _tag: "mp", leftIndex: 107, rightIndex: 110 },
    { _tag: "mp", leftIndex: 111, rightIndex: 109 },
    { _tag: "mp", leftIndex: 108, rightIndex: 112 },
    {
      _tag: "axiom",
      formulaText:
        "((~phi -> chi) -> ((~chi -> ~phi) -> (~chi -> chi))) -> (((~phi -> chi) -> (~chi -> ~phi)) -> ((~phi -> chi) -> (~chi -> chi)))",
    },
    { _tag: "mp", leftIndex: 113, rightIndex: 114 },
    {
      _tag: "axiom",
      formulaText: "((~chi -> ~phi)) -> ((~phi -> chi) -> (~chi -> ~phi))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((~chi -> ~phi) -> (((~phi -> chi) -> (~chi -> ~phi)) -> ((~phi -> chi) -> (~chi -> chi)))) -> (((~chi -> ~phi) -> ((~phi -> chi) -> (~chi -> ~phi))) -> ((~chi -> ~phi) -> ((~phi -> chi) -> (~chi -> chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((~phi -> chi) -> (~chi -> ~phi)) -> ((~phi -> chi) -> (~chi -> chi))) -> ((~chi -> ~phi) -> (((~phi -> chi) -> (~chi -> ~phi)) -> ((~phi -> chi) -> (~chi -> chi))))",
    },
    { _tag: "mp", leftIndex: 115, rightIndex: 118 },
    { _tag: "mp", leftIndex: 119, rightIndex: 117 },
    { _tag: "mp", leftIndex: 116, rightIndex: 120 },
    {
      _tag: "axiom",
      formulaText:
        "((phi -> chi) -> ((~chi -> ~phi) -> ((~phi -> chi) -> (~chi -> chi)))) -> (((phi -> chi) -> (~chi -> ~phi)) -> ((phi -> chi) -> ((~phi -> chi) -> (~chi -> chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((~chi -> ~phi) -> ((~phi -> chi) -> (~chi -> chi))) -> ((phi -> chi) -> ((~chi -> ~phi) -> ((~phi -> chi) -> (~chi -> chi))))",
    },
    { _tag: "mp", leftIndex: 121, rightIndex: 123 },
    { _tag: "mp", leftIndex: 124, rightIndex: 122 },
    { _tag: "mp", leftIndex: 106, rightIndex: 125 },
    ...
    { _tag: "mp", leftIndex: 148, rightIndex: 143 },
    {
      _tag: "axiom",
      formulaText:
        "(((~phi -> chi) -> ((~chi -> chi) -> chi))) -> (((~phi -> chi) -> (~chi -> chi)) -> ((~phi -> chi) -> chi))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((~chi -> chi) -> chi)) -> (((~phi -> chi) -> ((~chi -> chi) -> chi)))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((~chi -> chi) -> chi) -> (((~phi -> chi) -> ((~chi -> chi) -> chi)) -> (((~phi -> chi) -> (~chi -> chi)) -> ((~phi -> chi) -> chi)))) -> ((((~chi -> chi) -> chi) -> ((~phi -> chi) -> ((~chi -> chi) -> chi))) -> (((~chi -> chi) -> chi) -> (((~phi -> chi) -> (~chi -> chi)) -> ((~phi -> chi) -> chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((~phi -> chi) -> ((~chi -> chi) -> chi)) -> (((~phi -> chi) -> (~chi -> chi)) -> ((~phi -> chi) -> chi))) -> (((~chi -> chi) -> chi) -> (((~phi -> chi) -> ((~chi -> chi) -> chi)) -> (((~phi -> chi) -> (~chi -> chi)) -> ((~phi -> chi) -> chi))))",
    },
    { _tag: "mp", leftIndex: 150, rightIndex: 153 },
    { _tag: "mp", leftIndex: 154, rightIndex: 152 },
    { _tag: "mp", leftIndex: 151, rightIndex: 155 },
    { _tag: "mp", leftIndex: 149, rightIndex: 156 },
    {
      _tag: "axiom",
      formulaText:
        "((phi -> chi) -> (((~phi -> chi) -> (~chi -> chi)) -> ((~phi -> chi) -> chi))) -> (((phi -> chi) -> ((~phi -> chi) -> (~chi -> chi))) -> ((phi -> chi) -> ((~phi -> chi) -> chi)))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((~phi -> chi) -> (~chi -> chi)) -> ((~phi -> chi) -> chi)) -> ((phi -> chi) -> (((~phi -> chi) -> (~chi -> chi)) -> ((~phi -> chi) -> chi)))",
    },
    { _tag: "mp", leftIndex: 157, rightIndex: 159 },
    { _tag: "mp", leftIndex: 160, rightIndex: 158 },
    { _tag: "mp", leftIndex: 126, rightIndex: 161 },
    ...
    { _tag: "mp", leftIndex: 163, rightIndex: 166 },
    { _tag: "mp", leftIndex: 167, rightIndex: 165 },
    { _tag: "mp", leftIndex: 164, rightIndex: 168 },
    {
      _tag: "axiom",
      formulaText:
        "(((~phi -> psi) -> ((~phi -> chi) -> chi)) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((~phi -> chi) -> chi) -> ((~phi -> psi) -> ((~phi -> chi) -> chi)))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((~phi -> chi) -> chi) -> (((~phi -> psi) -> ((~phi -> chi) -> chi)) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)))) -> ((((~phi -> chi) -> chi) -> ((~phi -> psi) -> ((~phi -> chi) -> chi))) -> (((~phi -> chi) -> chi) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((~phi -> psi) -> ((~phi -> chi) -> chi)) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi))) -> (((~phi -> chi) -> chi) -> (((~phi -> psi) -> ((~phi -> chi) -> chi)) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)))))",
    },
    { _tag: "mp", leftIndex: 170, rightIndex: 173 },
    { _tag: "mp", leftIndex: 174, rightIndex: 172 },
    { _tag: "mp", leftIndex: 171, rightIndex: 175 },
    {
      _tag: "axiom",
      formulaText:
        "(((psi -> chi) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi))) -> (((psi -> chi) -> ((~phi -> psi) -> (~phi -> chi))) -> ((psi -> chi) -> ((~phi -> psi) -> chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> ((psi -> chi) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> (((psi -> chi) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi))) -> (((psi -> chi) -> ((~phi -> psi) -> (~phi -> chi))) -> ((psi -> chi) -> ((~phi -> psi) -> chi))))) -> (((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> ((psi -> chi) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)))) -> ((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> (((psi -> chi) -> ((~phi -> psi) -> (~phi -> chi))) -> ((psi -> chi) -> ((~phi -> psi) -> chi))))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((psi -> chi) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi))) -> (((psi -> chi) -> ((~phi -> psi) -> (~phi -> chi))) -> ((psi -> chi) -> ((~phi -> psi) -> chi)))) -> ((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> (((psi -> chi) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi))) -> (((psi -> chi) -> ((~phi -> psi) -> (~phi -> chi))) -> ((psi -> chi) -> ((~phi -> psi) -> chi))))))",
    },
    { _tag: "mp", leftIndex: 177, rightIndex: 180 },
    { _tag: "mp", leftIndex: 181, rightIndex: 179 },
    { _tag: "mp", leftIndex: 178, rightIndex: 182 },
    {
      _tag: "axiom",
      formulaText:
        "(((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> (((psi -> chi) -> ((~phi -> psi) -> (~phi -> chi))) -> ((psi -> chi) -> ((~phi -> psi) -> chi)))) -> (((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> ((psi -> chi) -> ((~phi -> psi) -> (~phi -> chi)))) -> ((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> ((psi -> chi) -> ((~phi -> psi) -> chi)))))",
    },
    { _tag: "mp", leftIndex: 183, rightIndex: 184 },
    {
      _tag: "axiom",
      formulaText:
        "(((psi -> chi) -> ((~phi -> psi) -> (~phi -> chi))) -> ((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> ((psi -> chi) -> ((~phi -> psi) -> (~phi -> chi)))))",
    },
    { _tag: "mp", leftIndex: 169, rightIndex: 186 },
    { _tag: "mp", leftIndex: 187, rightIndex: 185 },
    {
      _tag: "axiom",
      formulaText:
        "((((~phi -> chi) -> chi) -> ((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> ((psi -> chi) -> ((~phi -> psi) -> chi)))) -> ((((~phi -> chi) -> chi) -> (((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi))) -> (((~phi -> chi) -> chi) -> ((psi -> chi) -> ((~phi -> psi) -> chi)))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> ((psi -> chi) -> ((~phi -> psi) -> chi))) -> (((~phi -> chi) -> chi) -> ((((~phi -> psi) -> (~phi -> chi)) -> ((~phi -> psi) -> chi)) -> ((psi -> chi) -> ((~phi -> psi) -> chi)))))",
    },
    { _tag: "mp", leftIndex: 188, rightIndex: 190 },
    { _tag: "mp", leftIndex: 191, rightIndex: 189 },
    { _tag: "mp", leftIndex: 176, rightIndex: 192 },
    {
      _tag: "axiom",
      formulaText:
        "(((phi -> chi) -> (((~phi -> chi) -> chi) -> ((psi -> chi) -> ((~phi -> psi) -> chi)))) -> (((phi -> chi) -> ((~phi -> chi) -> chi)) -> ((phi -> chi) -> ((psi -> chi) -> ((~phi -> psi) -> chi)))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((~phi -> chi) -> chi) -> ((psi -> chi) -> ((~phi -> psi) -> chi))) -> ((phi -> chi) -> (((~phi -> chi) -> chi) -> ((psi -> chi) -> ((~phi -> psi) -> chi)))))",
    },
    { _tag: "mp", leftIndex: 193, rightIndex: 195 },
    { _tag: "mp", leftIndex: 196, rightIndex: 194 },
    { _tag: "mp", leftIndex: 162, rightIndex: 197 },
    {
      _tag: "axiom",
      formulaText:
        "(((~phi -> psi) -> (((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi))) -> (((~phi -> psi) -> ((psi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((psi -> chi) -> chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> ((~phi -> psi) -> (((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> (((~phi -> psi) -> (((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi))) -> (((~phi -> psi) -> ((psi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((psi -> chi) -> chi))))) -> (((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> ((~phi -> psi) -> (((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)))) -> ((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> (((~phi -> psi) -> ((psi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((psi -> chi) -> chi))))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((~phi -> psi) -> (((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi))) -> (((~phi -> psi) -> ((psi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((psi -> chi) -> chi)))) -> ((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> (((~phi -> psi) -> (((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi))) -> (((~phi -> psi) -> ((psi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((psi -> chi) -> chi))))))",
    },
    { _tag: "mp", leftIndex: 199, rightIndex: 202 },
    { _tag: "mp", leftIndex: 203, rightIndex: 201 },
    { _tag: "mp", leftIndex: 200, rightIndex: 204 },
    {
      _tag: "axiom",
      formulaText:
        "(((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> (((~phi -> psi) -> ((psi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((psi -> chi) -> chi)))) -> (((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> ((~phi -> psi) -> ((psi -> chi) -> (~phi -> psi)))) -> ((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> ((~phi -> psi) -> ((psi -> chi) -> chi)))))",
    },
    { _tag: "mp", leftIndex: 205, rightIndex: 206 },
    {
      _tag: "axiom",
      formulaText:
        "(((~phi -> psi) -> ((psi -> chi) -> (~phi -> psi))) -> ((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> ((~phi -> psi) -> ((psi -> chi) -> (~phi -> psi)))))",
    },
    {
      _tag: "axiom",
      formulaText: "((~phi -> psi) -> ((psi -> chi) -> (~phi -> psi)))",
    },
    { _tag: "mp", leftIndex: 209, rightIndex: 208 },
    { _tag: "mp", leftIndex: 210, rightIndex: 207 },
    {
      _tag: "axiom",
      formulaText:
        "(((psi -> chi) -> ((~phi -> psi) -> chi)) -> (((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((psi -> chi) -> ((~phi -> psi) -> chi)) -> ((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> ((~phi -> psi) -> ((psi -> chi) -> chi)))) -> ((((psi -> chi) -> ((~phi -> psi) -> chi)) -> (((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi))) -> (((psi -> chi) -> ((~phi -> psi) -> chi)) -> ((~phi -> psi) -> ((psi -> chi) -> chi)))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> ((~phi -> psi) -> ((psi -> chi) -> chi))) -> (((psi -> chi) -> ((~phi -> psi) -> chi)) -> ((((psi -> chi) -> (~phi -> psi)) -> ((psi -> chi) -> chi)) -> ((~phi -> psi) -> ((psi -> chi) -> chi)))))",
    },
    { _tag: "mp", leftIndex: 211, rightIndex: 214 },
    { _tag: "mp", leftIndex: 215, rightIndex: 213 },
    { _tag: "mp", leftIndex: 212, rightIndex: 216 },
    {
      _tag: "axiom",
      formulaText:
        "(((phi -> chi) -> (((psi -> chi) -> ((~phi -> psi) -> chi)) -> ((~phi -> psi) -> ((psi -> chi) -> chi)))) -> (((phi -> chi) -> ((psi -> chi) -> ((~phi -> psi) -> chi))) -> ((phi -> chi) -> ((~phi -> psi) -> ((psi -> chi) -> chi)))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((psi -> chi) -> ((~phi -> psi) -> chi)) -> ((~phi -> psi) -> ((psi -> chi) -> chi))) -> ((phi -> chi) -> (((psi -> chi) -> ((~phi -> psi) -> chi)) -> ((~phi -> psi) -> ((psi -> chi) -> chi)))))",
    },
    { _tag: "mp", leftIndex: 217, rightIndex: 219 },
    { _tag: "mp", leftIndex: 220, rightIndex: 218 },
    { _tag: "mp", leftIndex: 198, rightIndex: 221 },
    {
      _tag: "axiom",
      formulaText:
        "(((~phi -> psi) -> (((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi)))) -> (((~phi -> psi) -> ((phi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi)))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> ((~phi -> psi) -> (((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi)))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> (((~phi -> psi) -> (((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi)))) -> (((~phi -> psi) -> ((phi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi)))))) -> (((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> ((~phi -> psi) -> (((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))))) -> ((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> (((~phi -> psi) -> ((phi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi)))))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((~phi -> psi) -> (((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi)))) -> (((~phi -> psi) -> ((phi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi))))) -> ((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> (((~phi -> psi) -> (((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi)))) -> (((~phi -> psi) -> ((phi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi)))))))",
    },
    { _tag: "mp", leftIndex: 223, rightIndex: 226 },
    { _tag: "mp", leftIndex: 227, rightIndex: 225 },
    { _tag: "mp", leftIndex: 224, rightIndex: 228 },
    {
      _tag: "axiom",
      formulaText:
        "(((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> (((~phi -> psi) -> ((phi -> chi) -> (~phi -> psi))) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi))))) -> (((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> ((~phi -> psi) -> ((phi -> chi) -> (~phi -> psi)))) -> ((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi))))))",
    },
    { _tag: "mp", leftIndex: 229, rightIndex: 230 },
    {
      _tag: "axiom",
      formulaText:
        "(((~phi -> psi) -> ((phi -> chi) -> (~phi -> psi))) -> ((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> ((~phi -> psi) -> ((phi -> chi) -> (~phi -> psi)))))",
    },
    {
      _tag: "axiom",
      formulaText: "((~phi -> psi) -> ((phi -> chi) -> (~phi -> psi)))",
    },
    { _tag: "mp", leftIndex: 233, rightIndex: 232 },
    { _tag: "mp", leftIndex: 234, rightIndex: 231 },
    {
      _tag: "axiom",
      formulaText:
        "(((phi -> chi) -> ((~phi -> psi) -> ((psi -> chi) -> chi))) -> (((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "((((phi -> chi) -> ((~phi -> psi) -> ((psi -> chi) -> chi))) -> ((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi))))) -> ((((phi -> chi) -> ((~phi -> psi) -> ((psi -> chi) -> chi))) -> (((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi)))) -> (((phi -> chi) -> ((~phi -> psi) -> ((psi -> chi) -> chi))) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi))))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi)))) -> (((phi -> chi) -> ((~phi -> psi) -> ((psi -> chi) -> chi))) -> ((((phi -> chi) -> (~phi -> psi)) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi))))))",
    },
    { _tag: "mp", leftIndex: 235, rightIndex: 238 },
    { _tag: "mp", leftIndex: 239, rightIndex: 237 },
    { _tag: "mp", leftIndex: 236, rightIndex: 240 },
    { _tag: "mp", leftIndex: 222, rightIndex: 241 },
    { _tag: "axiom", formulaText: "(phi \\/ psi) -> (~phi -> psi)" },
    {
      _tag: "axiom",
      formulaText:
        "(((phi \\/ psi) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi)))) -> (((phi \\/ psi) -> (~phi -> psi)) -> ((phi \\/ psi) -> ((phi -> chi) -> ((psi -> chi) -> chi)))))",
    },
    {
      _tag: "axiom",
      formulaText:
        "(((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi))) -> ((phi \\/ psi) -> ((~phi -> psi) -> ((phi -> chi) -> ((psi -> chi) -> chi)))))",
    },
    { _tag: "mp", leftIndex: 242, rightIndex: 245 },
    { _tag: "mp", leftIndex: 246, rightIndex: 244 },
    { _tag: "mp", leftIndex: 243, rightIndex: 247 },
  ],
};
...
/**
 * prop-45: 選言の交換律 (φ ∨ ψ) → (ψ ∨ φ)
 *
 * 選言の定義: φ ∨ ψ ≡ ¬φ → ψ
 * ゴール（→/¬形式）: (¬φ → ψ) → (¬ψ → φ)
 *
 * 証明戦略（59ステップ）:
 * 1. DNI for ψ [37 steps]
 * 2. Inner compose: (¬φ → ψ) → (¬φ → ¬¬ψ) [4 steps]
 * 3. A3[φ:=φ, ψ:=¬ψ] + compose: (¬φ → ψ) → (¬ψ → φ) [6 steps]
 * 4. DISJ-DEF forward/backward + compose: (φ ∨ ψ) → (ψ ∨ φ) [12 steps]
 */
const prop45DisjComm: ModelAnswer = {
  questId: "prop-45",
  steps: [
    // --- DNI for ψ: prop-25 inline, 37 steps ---
    // 0.
    { _tag: "axiom", formulaText: "~~~psi -> (~~psi -> ~~~psi)" },
    // 1.
    {
      _tag: "axiom",
      formulaText: "(~~psi -> ~~~psi) -> (~~psi -> ~psi)",
    },
    // 2.
    {
      _tag: "axiom",
    ...
    // 44. MP(41, 43)
    { _tag: "mp", leftIndex: 41, rightIndex: 43 },
    // 45. MP(44, 42)
    { _tag: "mp", leftIndex: 44, rightIndex: 42 },
    // 46. MP(40, 45): (¬φ → ψ) → (¬ψ → φ)
    { _tag: "mp", leftIndex: 40, rightIndex: 45 },
    // --- DISJ-DEF forward + compose: (φ ∨ ψ) → (¬ψ → φ) ---
    // 47. DISJ-DEF forward
    {
      _tag: "axiom",
      formulaText: "(phi \\/ psi) -> (~phi -> psi)",
    ...
    // 52. MP(47, 51): (φ ∨ ψ) → (¬ψ → φ)
    { _tag: "mp", leftIndex: 47, rightIndex: 51 },
    // --- DISJ-DEF backward + compose: (φ ∨ ψ) → (ψ ∨ φ) ---
    // 53. DISJ-DEF backward
    {
      _tag: "axiom",
      formulaText: "(~psi -> phi) -> (psi \\/ phi)",
    },
    // 54. A2
    {
      _tag: "axiom",
      formulaText:
        "((phi \\/ psi) -> ((~psi -> phi) -> (psi \\/ phi))) -> (((phi \\/ psi) -> (~psi -> phi)) -> ((phi \\/ psi) -> (psi \\/ phi)))",
    },
    // 55. A1
    {
      _tag: "axiom",
      formulaText:
        "((~psi -> phi) -> (psi \\/ phi)) -> ((phi \\/ psi) -> ((~psi -> phi) -> (psi \\/ phi)))",
    },
    // 56. MP(53, 55)
    { _tag: "mp", leftIndex: 53, rightIndex: 55 },
    // 57. MP(56, 54)
    { _tag: "mp", leftIndex: 56, rightIndex: 54 },
    // 58. MP(52, 57): (φ ∨ ψ) → (ψ ∨ φ)
    { _tag: "mp", leftIndex: 52, rightIndex: 57 },
  ],
};

/**
 * prop-46: 連言の可換性 (φ ∧ ψ) → (ψ ∧ φ)
 *
 * 戦略: 257ステップ
 * 1. DNI for φ (prop-25 inline, 37 steps): φ → ¬¬φ
 * 2. MT inline with α=ψ, β=¬φ (prop-16, 107 steps): (ψ→¬φ) → (¬¬φ→¬ψ)
 * 3. Compose DNI + MT (17 steps): (ψ→¬φ) → (φ→¬ψ)
 *    - flipped B-combinator (12 steps): (¬¬φ→¬ψ) → (φ→¬ψ)
 *    - standard compose (5 steps): (ψ→¬φ) → (φ→¬ψ)
 * 4. DNE for (ψ→¬φ) (prop-17 inline, 35 steps): ¬¬(ψ→¬φ) → (ψ→¬φ)
 * 5. DNI for (φ→¬ψ) (prop-25 inline, 37 steps): (φ→¬ψ) → ¬¬(φ→¬ψ)
 * 6. Triple compose (10 steps): ¬¬(ψ→¬φ) → ¬¬(φ→¬ψ)
 * 7. A3 + MP (2 steps): ¬(φ→¬ψ) → ¬(ψ→¬φ)
 * 8. CONJ-DEF forward + backward + compose (12 steps): (φ∧ψ) → (ψ∧φ)
 */
const prop46ConjComm: ModelAnswer = {
  questId: "prop-46",
  steps: [
    // --- DNI for φ: prop-25 inline, 37 steps ---
    // 0.
    { _tag: "axiom", formulaText: "~~~phi -> (~~phi -> ~~~phi)" },
    // 1.
    { _tag: "axiom", formulaText: "(~~phi -> ~~~phi) -> (~~phi -> ~phi)" },
    ...
    // 36. MP(34, 35)
    { _tag: "mp", leftIndex: 34, rightIndex: 35 },
    // --- MT inline with α=ψ, β=¬φ: (ψ→¬φ) → (¬¬φ→¬ψ), 107 steps ---
    // 37.
    { _tag: "axiom", formulaText: "~~psi -> (~psi -> ~~psi)" },
    // 38.
    { _tag: "axiom", formulaText: "(~psi -> ~~psi) -> (~psi -> psi)" },
    ...
    { _tag: "mp", leftIndex: 64, rightIndex: 63 },
    // 66. MP(65, 60)
    { _tag: "mp", leftIndex: 65, rightIndex: 60 },
    // 67.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 71. MP(43, 70)
    { _tag: "mp", leftIndex: 43, rightIndex: 70 },
    // 72.
    { _tag: "axiom", formulaText: "~~~~phi -> (~~~phi -> ~~~~phi)" },
    // 73.
    { _tag: "axiom", formulaText: "(~~~phi -> ~~~~phi) -> (~~~phi -> ~~phi)" },
    ...
    { _tag: "mp", leftIndex: 76, rightIndex: 74 },
    // 78. MP(72, 77)
    { _tag: "mp", leftIndex: 72, rightIndex: 77 },
    // 79.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 83. MP(79, 82)
    { _tag: "mp", leftIndex: 79, rightIndex: 82 },
    // 84. MP(83, 81)
    { _tag: "mp", leftIndex: 83, rightIndex: 81 },
    // 85. MP(80, 84)
    { _tag: "mp", leftIndex: 80, rightIndex: 84 },
    // 86.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 91. MP(88, 90)
    { _tag: "mp", leftIndex: 88, rightIndex: 90 },
    // 92. MP(91, 89)
    { _tag: "mp", leftIndex: 91, rightIndex: 89 },
    // 93. MP(87, 92)
    { _tag: "mp", leftIndex: 87, rightIndex: 92 },
    // 94.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 95. MP(93, 94)
    { _tag: "mp", leftIndex: 93, rightIndex: 94 },
    // 96.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 98. MP(97, 96)
    { _tag: "mp", leftIndex: 97, rightIndex: 96 },
    // 99.
    {
      _tag: "axiom",
      formulaText:
    ...
    { _tag: "mp", leftIndex: 99, rightIndex: 98 },
    // 101. MP(100, 95)
    { _tag: "mp", leftIndex: 100, rightIndex: 95 },
    // 102.
    {
      _tag: "axiom",
      formulaText:
    ...
    { _tag: "mp", leftIndex: 104, rightIndex: 102 },
    // 106. MP(78, 105)
    { _tag: "mp", leftIndex: 78, rightIndex: 105 },
    // 107.
    { _tag: "axiom", formulaText: "(~~~~phi -> ~~phi) -> (~phi -> ~~~phi)" },
    // 108. MP(106, 107)
    ...
    // 109.
    {
      _tag: "axiom",
      formulaText:
        "((~~psi -> (psi -> ~phi))) -> ((~~psi -> psi) -> (~~psi -> ~phi))",
    },
    // 110.
    {
      _tag: "axiom",
      formulaText: "((psi -> ~phi)) -> ((~~psi -> (psi -> ~phi)))",
    },
    // 111.
    {
      _tag: "axiom",
      formulaText:
        "((psi -> ~phi) -> ((~~psi -> (psi -> ~phi)) -> ((~~psi -> psi) -> (~~psi -> ~phi)))) -> (((psi -> ~phi) -> (~~psi -> (psi -> ~phi))) -> ((psi -> ~phi) -> ((~~psi -> psi) -> (~~psi -> ~phi))))",
    },
    // 112.
    {
      _tag: "axiom",
      formulaText:
        "((~~psi -> (psi -> ~phi)) -> ((~~psi -> psi) -> (~~psi -> ~phi))) -> ((psi -> ~phi) -> ((~~psi -> (psi -> ~phi)) -> ((~~psi -> psi) -> (~~psi -> ~phi))))",
    },
    // 113. MP(109, 112)
    { _tag: "mp", leftIndex: 109, rightIndex: 112 },
    // 114. MP(113, 111)
    { _tag: "mp", leftIndex: 113, rightIndex: 111 },
    // 115. MP(110, 114)
    { _tag: "mp", leftIndex: 110, rightIndex: 114 },
    // 116.
    {
      _tag: "axiom",
      formulaText:
        "((psi -> ~phi) -> ((~~psi -> psi) -> (~~psi -> ~phi))) -> (((psi -> ~phi) -> (~~psi -> psi)) -> ((psi -> ~phi) -> (~~psi -> ~phi)))",
    },
    // 117. MP(115, 116)
    { _tag: "mp", leftIndex: 115, rightIndex: 116 },
    // 118.
    {
      _tag: "axiom",
      formulaText: "((~~psi -> psi)) -> ((psi -> ~phi) -> (~~psi -> psi))",
    },
    // 119.
    {
      _tag: "axiom",
      formulaText:
        "((~~psi -> psi) -> (((psi -> ~phi) -> (~~psi -> psi)) -> ((psi -> ~phi) -> (~~psi -> ~phi)))) -> (((~~psi -> psi) -> ((psi -> ~phi) -> (~~psi -> psi))) -> ((~~psi -> psi) -> ((psi -> ~phi) -> (~~psi -> ~phi))))",
    },
    // 120.
    {
      _tag: "axiom",
      formulaText:
        "(((psi -> ~phi) -> (~~psi -> psi)) -> ((psi -> ~phi) -> (~~psi -> ~phi))) -> ((~~psi -> psi) -> (((psi -> ~phi) -> (~~psi -> psi)) -> ((psi -> ~phi) -> (~~psi -> ~phi))))",
    },
    // 121. MP(117, 120)
    { _tag: "mp", leftIndex: 117, rightIndex: 120 },
    // 122. MP(121, 119)
    { _tag: "mp", leftIndex: 121, rightIndex: 119 },
    // 123. MP(118, 122)
    { _tag: "mp", leftIndex: 118, rightIndex: 122 },
    // 124. MP(71, 123)
    { _tag: "mp", leftIndex: 71, rightIndex: 123 },
    // 125.
    {
      _tag: "axiom",
      formulaText:
        "((~~psi -> (~phi -> ~~~phi))) -> ((~~psi -> ~phi) -> (~~psi -> ~~~phi))",
    },
    // 126.
    {
      _tag: "axiom",
    ...
    // 127.
    {
      _tag: "axiom",
      formulaText:
        "((~phi -> ~~~phi) -> ((~~psi -> (~phi -> ~~~phi)) -> ((~~psi -> ~phi) -> (~~psi -> ~~~phi)))) -> (((~phi -> ~~~phi) -> (~~psi -> (~phi -> ~~~phi))) -> ((~phi -> ~~~phi) -> ((~~psi -> ~phi) -> (~~psi -> ~~~phi))))",
    },
    // 128.
    {
      _tag: "axiom",
      formulaText:
        "((~~psi -> (~phi -> ~~~phi)) -> ((~~psi -> ~phi) -> (~~psi -> ~~~phi))) -> ((~phi -> ~~~phi) -> ((~~psi -> (~phi -> ~~~phi)) -> ((~~psi -> ~phi) -> (~~psi -> ~~~phi))))",
    },
    // 129. MP(125, 128)
    { _tag: "mp", leftIndex: 125, rightIndex: 128 },
    // 130. MP(129, 127)
    { _tag: "mp", leftIndex: 129, rightIndex: 127 },
    // 131. MP(126, 130)
    { _tag: "mp", leftIndex: 126, rightIndex: 130 },
    // 132. MP(108, 131)
    { _tag: "mp", leftIndex: 108, rightIndex: 131 },
    // 133.
    {
      _tag: "axiom",
      formulaText:
        "((psi -> ~phi) -> ((~~psi -> ~phi) -> (~~psi -> ~~~phi))) -> (((psi -> ~phi) -> (~~psi -> ~phi)) -> ((psi -> ~phi) -> (~~psi -> ~~~phi)))",
    },
    // 134.
    {
      _tag: "axiom",
    ...
    // 135. MP(132, 134)
    { _tag: "mp", leftIndex: 132, rightIndex: 134 },
    // 136. MP(135, 133)
    { _tag: "mp", leftIndex: 135, rightIndex: 133 },
    // 137. MP(124, 136)
    { _tag: "mp", leftIndex: 124, rightIndex: 136 },
    // 138.
    { _tag: "axiom", formulaText: "(~~psi -> ~~~phi) -> (~~phi -> ~psi)" },
    // 139.
    {
      _tag: "axiom",
      formulaText:
        "((psi -> ~phi) -> ((~~psi -> ~~~phi) -> (~~phi -> ~psi))) -> (((psi -> ~phi) -> (~~psi -> ~~~phi)) -> ((psi -> ~phi) -> (~~phi -> ~psi)))",
    },
    // 140.
    {
      _tag: "axiom",
      formulaText:
        "((~~psi -> ~~~phi) -> (~~phi -> ~psi)) -> ((psi -> ~phi) -> ((~~psi -> ~~~phi) -> (~~phi -> ~psi)))",
    },
    // 141. MP(138, 140)
    { _tag: "mp", leftIndex: 138, rightIndex: 140 },
    // 142. MP(141, 139)
    { _tag: "mp", leftIndex: 141, rightIndex: 139 },
    // 143. MP(137, 142)
    { _tag: "mp", leftIndex: 137, rightIndex: 142 },
    // --- compose DNI + MT: (ψ→¬φ) → (φ→¬ψ) ---
    // 144.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 148. MP(144, 147)
    { _tag: "mp", leftIndex: 144, rightIndex: 147 },
    // 149. MP(148, 146)
    { _tag: "mp", leftIndex: 148, rightIndex: 146 },
    // 150. MP(145, 149)
    { _tag: "mp", leftIndex: 145, rightIndex: 149 },
    // 151.
    {
      _tag: "axiom",
      formulaText: "(phi -> ~~phi) -> ((~~phi -> ~psi) -> (phi -> ~~phi))",
    ...
    // 152. MP(36, 151)
    { _tag: "mp", leftIndex: 36, rightIndex: 151 },
    // 153.
    {
      _tag: "axiom",
      formulaText:
    ...
    { _tag: "mp", leftIndex: 150, rightIndex: 153 },
    // 155. MP(152, 154)
    { _tag: "mp", leftIndex: 152, rightIndex: 154 },
    // 156.
    {
      _tag: "axiom",
      formulaText:
        "((psi -> ~phi) -> ((~~phi -> ~psi) -> (phi -> ~psi))) -> (((psi -> ~phi) -> (~~phi -> ~psi)) -> ((psi -> ~phi) -> (phi -> ~psi)))",
    },
    // 157.
    {
      _tag: "axiom",
      formulaText:
        "((~~phi -> ~psi) -> (phi -> ~psi)) -> ((psi -> ~phi) -> ((~~phi -> ~psi) -> (phi -> ~psi)))",
    },
    // 158. MP(155, 157)
    { _tag: "mp", leftIndex: 155, rightIndex: 157 },
    // 159. MP(158, 156)
    { _tag: "mp", leftIndex: 158, rightIndex: 156 },
    // 160. MP(143, 159)
    { _tag: "mp", leftIndex: 143, rightIndex: 159 },
    // --- DNE for (ψ→¬φ): prop-17 inline, 35 steps ---
    // 161.
    {
      _tag: "axiom",
      formulaText: "~~(psi -> ~phi) -> (~(psi -> ~phi) -> ~~(psi -> ~phi))",
    },
    // 162.
    {
      _tag: "axiom",
      formulaText:
        "(~(psi -> ~phi) -> ~~(psi -> ~phi)) -> (~(psi -> ~phi) -> (psi -> ~phi))",
    },
    // 163.
    {
      _tag: "axiom",
      formulaText:
        "(~~(psi -> ~phi) -> ((~(psi -> ~phi) -> ~~(psi -> ~phi)) -> (~(psi -> ~phi) -> (psi -> ~phi)))) -> ((~~(psi -> ~phi) -> (~(psi -> ~phi) -> ~~(psi -> ~phi))) -> (~~(psi -> ~phi) -> (~(psi -> ~phi) -> (psi -> ~phi))))",
    },
    // 164.
    {
      _tag: "axiom",
      formulaText:
        "((~(psi -> ~phi) -> ~~(psi -> ~phi)) -> (~(psi -> ~phi) -> (psi -> ~phi))) -> (~~(psi -> ~phi) -> ((~(psi -> ~phi) -> ~~(psi -> ~phi)) -> (~(psi -> ~phi) -> (psi -> ~phi))))",
    },
    // 165. MP(162, 164)
    { _tag: "mp", leftIndex: 162, rightIndex: 164 },
    // 166. MP(165, 163)
    { _tag: "mp", leftIndex: 165, rightIndex: 163 },
    // 167. MP(161, 166)
    { _tag: "mp", leftIndex: 161, rightIndex: 166 },
    // 168.
    {
      _tag: "axiom",
      formulaText:
        "(~~(~(psi -> ~phi) -> (psi -> ~phi)) -> ~(psi -> ~phi)) -> ((psi -> ~phi) -> ~(~(psi -> ~phi) -> (psi -> ~phi)))",
    },
    // 169.
    {
      _tag: "axiom",
    ...
    // 170.
    {
      _tag: "axiom",
      formulaText:
        "(~(psi -> ~phi) -> ((~~(~(psi -> ~phi) -> (psi -> ~phi)) -> ~(psi -> ~phi)) -> ((psi -> ~phi) -> ~(~(psi -> ~phi) -> (psi -> ~phi))))) -> ((~(psi -> ~phi) -> (~~(~(psi -> ~phi) -> (psi -> ~phi)) -> ~(psi -> ~phi))) -> (~(psi -> ~phi) -> ((psi -> ~phi) -> ~(~(psi -> ~phi) -> (psi -> ~phi)))))",
    },
    // 171.
    {
      _tag: "axiom",
      formulaText:
        "((~~(~(psi -> ~phi) -> (psi -> ~phi)) -> ~(psi -> ~phi)) -> ((psi -> ~phi) -> ~(~(psi -> ~phi) -> (psi -> ~phi)))) -> (~(psi -> ~phi) -> ((~~(~(psi -> ~phi) -> (psi -> ~phi)) -> ~(psi -> ~phi)) -> ((psi -> ~phi) -> ~(~(psi -> ~phi) -> (psi -> ~phi)))))",
    },
    // 172. MP(168, 171)
    { _tag: "mp", leftIndex: 168, rightIndex: 171 },
    // 173. MP(172, 170)
    { _tag: "mp", leftIndex: 172, rightIndex: 170 },
    // 174. MP(169, 173)
    { _tag: "mp", leftIndex: 169, rightIndex: 173 },
    // 175.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 176. MP(174, 175)
    { _tag: "mp", leftIndex: 174, rightIndex: 175 },
    // 177.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 178.
    {
      _tag: "axiom",
      formulaText:
        "((~(psi -> ~phi) -> (psi -> ~phi)) -> ((~(psi -> ~phi) -> ~(~(psi -> ~phi) -> (psi -> ~phi))) -> ((~(psi -> ~phi) -> (psi -> ~phi)) -> (psi -> ~phi)))) -> (((~(psi -> ~phi) -> (psi -> ~phi)) -> (~(psi -> ~phi) -> ~(~(psi -> ~phi) -> (psi -> ~phi)))) -> ((~(psi -> ~phi) -> (psi -> ~phi)) -> ((~(psi -> ~phi) -> (psi -> ~phi)) -> (psi -> ~phi))))",
    },
    // 179.
    {
      _tag: "axiom",
      formulaText:
        "((~(psi -> ~phi) -> ~(~(psi -> ~phi) -> (psi -> ~phi))) -> ((~(psi -> ~phi) -> (psi -> ~phi)) -> (psi -> ~phi))) -> ((~(psi -> ~phi) -> (psi -> ~phi)) -> ((~(psi -> ~phi) -> ~(~(psi -> ~phi) -> (psi -> ~phi))) -> ((~(psi -> ~phi) -> (psi -> ~phi)) -> (psi -> ~phi))))",
    },
    // 180. MP(177, 179)
    { _tag: "mp", leftIndex: 177, rightIndex: 179 },
    // 181. MP(180, 178)
    { _tag: "mp", leftIndex: 180, rightIndex: 178 },
    // 182. MP(176, 181)
    ...
    // 183.
    {
      _tag: "axiom",
      formulaText:
        "((~(psi -> ~phi) -> (psi -> ~phi)) -> ((~(psi -> ~phi) -> (psi -> ~phi)) -> (psi -> ~phi))) -> (((~(psi -> ~phi) -> (psi -> ~phi)) -> (~(psi -> ~phi) -> (psi -> ~phi))) -> ((~(psi -> ~phi) -> (psi -> ~phi)) -> (psi -> ~phi)))",
    },
    // 184. MP(182, 183)
    { _tag: "mp", leftIndex: 182, rightIndex: 183 },
    // 185.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 186.
    {
      _tag: "axiom",
      formulaText:
        "(~(psi -> ~phi) -> (psi -> ~phi)) -> (((~(psi -> ~phi) -> (psi -> ~phi)) -> (~(psi -> ~phi) -> (psi -> ~phi))) -> (~(psi -> ~phi) -> (psi -> ~phi)))",
    },
    // 187. MP(186, 185)
    { _tag: "mp", leftIndex: 186, rightIndex: 185 },
    // 188.
    ...
    // 189. MP(188, 187)
    { _tag: "mp", leftIndex: 188, rightIndex: 187 },
    // 190. MP(189, 184)
    { _tag: "mp", leftIndex: 189, rightIndex: 184 },
    // 191.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 193. MP(190, 192)
    { _tag: "mp", leftIndex: 190, rightIndex: 192 },
    // 194. MP(193, 191)
    { _tag: "mp", leftIndex: 193, rightIndex: 191 },
    // 195. MP(167, 194)
    { _tag: "mp", leftIndex: 167, rightIndex: 194 },
    // --- DNI for (φ→¬ψ): prop-25 inline, 37 steps ---
    // 196.
    {
      _tag: "axiom",
    ...
    // 200. MP(197, 199)
    { _tag: "mp", leftIndex: 197, rightIndex: 199 },
    // 201. MP(200, 198)
    { _tag: "mp", leftIndex: 200, rightIndex: 198 },
    // 202. MP(196, 201)
    { _tag: "mp", leftIndex: 196, rightIndex: 201 },
    // 203.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 207. MP(203, 206)
    { _tag: "mp", leftIndex: 203, rightIndex: 206 },
    // 208. MP(207, 205)
    { _tag: "mp", leftIndex: 207, rightIndex: 205 },
    // 209. MP(204, 208)
    { _tag: "mp", leftIndex: 204, rightIndex: 208 },
    // 210.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 215. MP(212, 214)
    { _tag: "mp", leftIndex: 212, rightIndex: 214 },
    // 216. MP(215, 213)
    { _tag: "mp", leftIndex: 215, rightIndex: 213 },
    // 217. MP(211, 216)
    { _tag: "mp", leftIndex: 211, rightIndex: 216 },
    // 218.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 219. MP(217, 218)
    { _tag: "mp", leftIndex: 217, rightIndex: 218 },
    // 220.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 222. MP(221, 220)
    { _tag: "mp", leftIndex: 221, rightIndex: 220 },
    // 223.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 224. MP(223, 222)
    { _tag: "mp", leftIndex: 223, rightIndex: 222 },
    // 225. MP(224, 219)
    { _tag: "mp", leftIndex: 224, rightIndex: 219 },
    // 226.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 228. MP(225, 227)
    { _tag: "mp", leftIndex: 225, rightIndex: 227 },
    // 229. MP(228, 226)
    { _tag: "mp", leftIndex: 228, rightIndex: 226 },
    // 230. MP(202, 229)
    { _tag: "mp", leftIndex: 202, rightIndex: 229 },
    // 231.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 235. MP(160, 234)
    { _tag: "mp", leftIndex: 160, rightIndex: 234 },
    // 236. MP(235, 233)
    { _tag: "mp", leftIndex: 235, rightIndex: 233 },
    // 237. MP(195, 236)
    { _tag: "mp", leftIndex: 195, rightIndex: 236 },
    // 238.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 240. MP(232, 239)
    { _tag: "mp", leftIndex: 232, rightIndex: 239 },
    // 241. MP(240, 238)
    { _tag: "mp", leftIndex: 240, rightIndex: 238 },
    // 242. MP(237, 241)
    { _tag: "mp", leftIndex: 237, rightIndex: 241 },
    // --- A3 + MP: ¬(φ→¬ψ) → ¬(ψ→¬φ) ---
    // 243.
    {
      _tag: "axiom",
    ...
    // 244. MP(242, 243)
    { _tag: "mp", leftIndex: 242, rightIndex: 243 },
    // --- CONJ-DEF forward + backward + compose: (φ∧ψ) → (ψ∧φ) ---
    // 245.
    { _tag: "axiom", formulaText: "(phi /\\ psi) -> ~(phi -> ~psi)" },
    // 246.
    {
    ...
    // 248. MP(244, 247)
    { _tag: "mp", leftIndex: 244, rightIndex: 247 },
    // 249. MP(248, 246)
    { _tag: "mp", leftIndex: 248, rightIndex: 246 },
    // 250. MP(245, 249)
    { _tag: "mp", leftIndex: 245, rightIndex: 249 },
    // 251.
    { _tag: "axiom", formulaText: "~(psi -> ~phi) -> (psi /\\ phi)" },
    // 252.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 254. MP(251, 253)
    { _tag: "mp", leftIndex: 251, rightIndex: 253 },
    // 255. MP(254, 252)
    { _tag: "mp", leftIndex: 254, rightIndex: 252 },
    // 256. MP(250, 255)
...
/**
 * prop-47: De Morgan converse: (¬φ ∧ ¬ψ) → ¬(φ ∨ ψ)
 *
 * 戦略: 161ステップ
 * 1. DNI for ψ (prop-25 inline, 37 steps): ψ → ¬¬ψ
 * 2. Inner compose (4 steps): (¬φ → ψ) → (¬φ → ¬¬ψ)
 * 3. DISJ-DEF forward + compose (6 steps): (φ ∨ ψ) → (¬φ → ¬¬ψ)
 * 4. MT inline (prop-16, 107 steps): ((φ∨ψ) → (¬φ→¬¬ψ)) → (¬(¬φ→¬¬ψ) → ¬(φ∨ψ))
 *    with φ:=(φ∨ψ), ψ:=(¬φ→¬¬ψ)
 * 5. MP + CONJ-DEF forward + compose (7 steps): (¬φ ∧ ¬ψ) → ¬(φ ∨ ψ)
 *    CONJ-DEF forward with α:=¬φ, β:=¬ψ gives (¬φ∧¬ψ) → ¬(¬φ→¬¬ψ)
 */
const prop47DeMorganConverse: ModelAnswer = {
  ...
  steps: [
    ...
    // 40. MP(39, 37)
    { _tag: "mp", leftIndex: 39, rightIndex: 37 },
    // --- DISJ-DEF forward + compose: (φ ∨ ψ) → (¬φ → ¬¬ψ) ---
    // 41.
    { _tag: "axiom", formulaText: "(phi \\/ psi) -> (~phi -> psi)" },
    // 42.
    {
    ...
    // 44. MP(40, 43)
    { _tag: "mp", leftIndex: 40, rightIndex: 43 },
    // 45. MP(44, 42)
    { _tag: "mp", leftIndex: 44, rightIndex: 42 },
    // 46. MP(41, 45)
    { _tag: "mp", leftIndex: 41, rightIndex: 45 },
    // --- MT inline: ((φ∨ψ) → (¬φ→¬¬ψ)) → (¬(¬φ→¬¬ψ) → ¬(φ∨ψ)), 107 steps ---
    // 47.
    {
      _tag: "axiom",
      formulaText: "~~(phi \\/ psi) -> (~(phi \\/ psi) -> ~~(phi \\/ psi))",
    ...
    // 52. MP(51, 49)
    { _tag: "mp", leftIndex: 51, rightIndex: 49 },
    // 53. MP(47, 52)
    { _tag: "mp", leftIndex: 47, rightIndex: 52 },
    // 54.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 58. MP(54, 57)
    { _tag: "mp", leftIndex: 54, rightIndex: 57 },
    // 59. MP(58, 56)
    { _tag: "mp", leftIndex: 58, rightIndex: 56 },
    // 60. MP(55, 59)
    { _tag: "mp", leftIndex: 55, rightIndex: 59 },
    // 61.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 66. MP(63, 65)
    { _tag: "mp", leftIndex: 63, rightIndex: 65 },
    // 67. MP(66, 64)
    { _tag: "mp", leftIndex: 66, rightIndex: 64 },
    // 68. MP(62, 67)
    { _tag: "mp", leftIndex: 62, rightIndex: 67 },
    // 69.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 73. MP(72, 71)
    { _tag: "mp", leftIndex: 72, rightIndex: 71 },
    // 74.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 75. MP(74, 73)
    { _tag: "mp", leftIndex: 74, rightIndex: 73 },
    // 76. MP(75, 70)
    { _tag: "mp", leftIndex: 75, rightIndex: 70 },
    // 77.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 79. MP(76, 78)
    { _tag: "mp", leftIndex: 76, rightIndex: 78 },
    // 80. MP(79, 77)
    { _tag: "mp", leftIndex: 79, rightIndex: 77 },
    // 81. MP(53, 80)
    { _tag: "mp", leftIndex: 53, rightIndex: 80 },
    // 82.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 86. MP(83, 85)
    { _tag: "mp", leftIndex: 83, rightIndex: 85 },
    // 87. MP(86, 84)
    { _tag: "mp", leftIndex: 86, rightIndex: 84 },
    // 88. MP(82, 87)
    { _tag: "mp", leftIndex: 82, rightIndex: 87 },
    // 89.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 93. MP(89, 92)
    { _tag: "mp", leftIndex: 89, rightIndex: 92 },
    // 94. MP(93, 91)
    { _tag: "mp", leftIndex: 93, rightIndex: 91 },
    // 95. MP(90, 94)
    { _tag: "mp", leftIndex: 90, rightIndex: 94 },
    // 96.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 101. MP(98, 100)
    { _tag: "mp", leftIndex: 98, rightIndex: 100 },
    // 102. MP(101, 99)
    { _tag: "mp", leftIndex: 101, rightIndex: 99 },
    // 103. MP(97, 102)
    { _tag: "mp", leftIndex: 97, rightIndex: 102 },
    // 104.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 108. MP(107, 106)
    { _tag: "mp", leftIndex: 107, rightIndex: 106 },
    // 109.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 110. MP(109, 108)
    { _tag: "mp", leftIndex: 109, rightIndex: 108 },
    // 111. MP(110, 105)
    { _tag: "mp", leftIndex: 110, rightIndex: 105 },
    // 112.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 114. MP(111, 113)
    { _tag: "mp", leftIndex: 111, rightIndex: 113 },
    // 115. MP(114, 112)
    { _tag: "mp", leftIndex: 114, rightIndex: 112 },
    // 116. MP(88, 115)
    { _tag: "mp", leftIndex: 88, rightIndex: 115 },
    // 117.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 121.
    {
      _tag: "axiom",
      formulaText:
        "(((phi \\/ psi) -> (~phi -> ~~psi)) -> ((~~(phi \\/ psi) -> ((phi \\/ psi) -> (~phi -> ~~psi))) -> ((~~(phi \\/ psi) -> (phi \\/ psi)) -> (~~(phi \\/ psi) -> (~phi -> ~~psi))))) -> ((((phi \\/ psi) -> (~phi -> ~~psi)) -> (~~(phi \\/ psi) -> ((phi \\/ psi) -> (~phi -> ~~psi)))) -> (((phi \\/ psi) -> (~phi -> ~~psi)) -> ((~~(phi \\/ psi) -> (phi \\/ psi)) -> (~~(phi \\/ psi) -> (~phi -> ~~psi)))))",
    },
    // 122.
    {
      _tag: "axiom",
      formulaText:
        "((~~(phi \\/ psi) -> ((phi \\/ psi) -> (~phi -> ~~psi))) -> ((~~(phi \\/ psi) -> (phi \\/ psi)) -> (~~(phi \\/ psi) -> (~phi -> ~~psi)))) -> (((phi \\/ psi) -> (~phi -> ~~psi)) -> ((~~(phi \\/ psi) -> ((phi \\/ psi) -> (~phi -> ~~psi))) -> ((~~(phi \\/ psi) -> (phi \\/ psi)) -> (~~(phi \\/ psi) -> (~phi -> ~~psi)))))",
    },
    // 123. MP(119, 122)
    ...
    // 129.
    {
      _tag: "axiom",
      formulaText:
        "((~~(phi \\/ psi) -> (phi \\/ psi)) -> ((((phi \\/ psi) -> (~phi -> ~~psi)) -> (~~(phi \\/ psi) -> (phi \\/ psi))) -> (((phi \\/ psi) -> (~phi -> ~~psi)) -> (~~(phi \\/ psi) -> (~phi -> ~~psi))))) -> (((~~(phi \\/ psi) -> (phi \\/ psi)) -> (((phi \\/ psi) -> (~phi -> ~~psi)) -> (~~(phi \\/ psi) -> (phi \\/ psi)))) -> ((~~(phi \\/ psi) -> (phi \\/ psi)) -> (((phi \\/ psi) -> (~phi -> ~~psi)) -> (~~(phi \\/ psi) -> (~phi -> ~~psi)))))",
    },
    // 130.
    {
      _tag: "axiom",
    ...
    { _tag: "mp", leftIndex: 128, rightIndex: 132 },
    // 134. MP(81, 133)
    { _tag: "mp", leftIndex: 81, rightIndex: 133 },
    // 135.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 139. MP(135, 138)
    { _tag: "mp", leftIndex: 135, rightIndex: 138 },
    // 140. MP(139, 137)
    { _tag: "mp", leftIndex: 139, rightIndex: 137 },
    // 141. MP(136, 140)
    { _tag: "mp", leftIndex: 136, rightIndex: 140 },
    // 142. MP(118, 141)
    { _tag: "mp", leftIndex: 118, rightIndex: 141 },
    // 143.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 144.
    {
      _tag: "axiom",
      formulaText:
        "((~~(phi \\/ psi) -> (~phi -> ~~psi)) -> (~~(phi \\/ psi) -> ~~(~phi -> ~~psi))) -> (((phi \\/ psi) -> (~phi -> ~~psi)) -> ((~~(phi \\/ psi) -> (~phi -> ~~psi)) -> (~~(phi \\/ psi) -> ~~(~phi -> ~~psi))))",
    },
    // 145. MP(142, 144)
    ...
    // 147. MP(134, 146)
    { _tag: "mp", leftIndex: 134, rightIndex: 146 },
    // 148.
    {
      _tag: "axiom",
      formulaText:
    ...
    // 151. MP(148, 150)
    { _tag: "mp", leftIndex: 148, rightIndex: 150 },
    // 152. MP(151, 149)
    { _tag: "mp", leftIndex: 151, rightIndex: 149 },
    // 153. MP(147, 152)
    { _tag: "mp", leftIndex: 147, rightIndex: 152 },
    // --- MP + CONJ-DEF forward + compose: (¬φ ∧ ¬ψ) → ¬(φ ∨ ψ) ---
    // 154. MP(46, 153)
    { _tag: "mp", leftIndex: 46, rightIndex: 153 },
    // 155.
    { _tag: "axiom", formulaText: "(~phi /\\ ~psi) -> ~(~phi -> ~~psi)" },
    // 156.
    {
    ...
    // 158. MP(154, 157)
    { _tag: "mp", leftIndex: 154, rightIndex: 157 },
    // 159. MP(158, 156)
    { _tag: "mp", leftIndex: 158, rightIndex: 156 },
    // 160. MP(155, 159)
    { _tag: "mp", leftIndex: 155, rightIndex: 159 },
  ],
};
...
/**
 * tab-19: 全称除去 ¬(∀x.P(x) → P(x))
 *
 * 0. Root: ¬(∀x.P(x) → P(x))
 * 1. ¬→(pos 0): [∀x.P(x), ¬P(x), ¬(∀x.P(x)→P(x))]
 * 2. ∀(pos 0, term "x"): [P(x), ∀x.P(x), ¬P(x), ¬(∀x.P(x)→P(x))]
 * 3. BS: P(x) と ¬P(x)
 */
...
/**
 * tab-20: 存在→否定全称否定 ¬(∃x.P(x) → ¬∀x.¬P(x))
 *
 * 0. Root: ¬(∃x.P(x) → ¬∀x.¬P(x))
 * 1. ¬→(pos 0): [∃x.P(x), ¬¬∀x.¬P(x), ¬(...root)]
 * 2. ¬¬(pos 1): [∀x.¬P(x), ¬¬∀x.¬P(x), ∃x.P(x), ¬(...root)]
 * 3. ∃(pos 2, eigen "a"): [P(a), ∃x.P(x), ∀x.¬P(x), ¬¬∀x.¬P(x), ¬(...root)]
 * 4. ∀(pos 2, term "a"): [¬P(a), ∀x.¬P(x), P(a), ∃x.P(x), ¬¬∀x.¬P(x), ¬(...root)]
 * 5. BS: P(a) と ¬P(a)
 */
...
/**
 * tab-21: 全称含意分配 ¬(∀x.(P(x)→Q(x)) → (∀x.P(x) → ∀x.Q(x)))
 *
 * 0. Root: ¬(∀x.(P(x)→Q(x)) → (∀x.P(x) → ∀x.Q(x)))
 * 1. ¬→(pos 0): [∀x.(P(x)→Q(x)), ¬(∀x.P(x) → ∀x.Q(x)), ¬(...root)]
 * 2. ¬→(pos 1): [∀x.P(x), ¬∀x.Q(x), ¬(∀x.P(x)→∀x.Q(x)), ∀x.(P(x)→Q(x)), ¬(...root)]
 * 3. ¬∀(pos 1, eigen "a"): [¬Q(a), ¬∀x.Q(x), ∀x.P(x), ¬(∀x.P(x)→∀x.Q(x)), ∀x.(P(x)→Q(x)), ¬(...root)]
 * 4. ∀(pos 2, term "a"): [P(a), ∀x.P(x), ¬Q(a), ¬∀x.Q(x), ¬(∀x.P(x)→∀x.Q(x)), ∀x.(P(x)→Q(x)), ¬(...root)]
 * 5. ∀(pos 5, term "a"): [P(a)→Q(a), ∀x.(P(x)→Q(x)), P(a), ∀x.P(x), ¬Q(a), ¬∀x.Q(x), ¬(...→...), ¬(...root)]
 * 6. →(pos 0): 分岐
 *    左(stepNodeIds[6]): [¬P(a), P(a)→Q(a), ...] → BS (¬P(a), P(a))
 *    右(stepNodeIds[7]): [Q(a), P(a)→Q(a), ...] → BS (Q(a), ¬Q(a))
 * 7. BS on left
 * 8. BS on right
 */
const tab21UniversalImplicationDist: ModelAnswer = {
  ...
  steps: [
    ...
    // → 分岐: stepNodeIds[6]=left(¬P(a)), stepNodeIds[7]=right(Q(a))
    {
      _tag: "tab-rule",
      conclusionIndex: 5,
      ruleId: "implication",
...
/**
 * tab-22: 全称連言分配 ¬(∀x.(P(x)∧Q(x)) → (∀x.P(x) ∧ ∀x.Q(x)))
 *
 * 0. Root: ¬(∀x.(P(x)∧Q(x)) → (∀x.P(x) ∧ ∀x.Q(x)))
 * 1. ¬→(pos 0): [∀x.(P(x)∧Q(x)), ¬(∀x.P(x)∧∀x.Q(x)), ¬(...root)]
 * 2. ¬∧(pos 1): 分岐
 *    左(stepNodeIds[2]): [¬∀x.P(x), ¬(∀x.P(x)∧∀x.Q(x)), ∀x.(P(x)∧Q(x)), ¬(...root)]
 *    右(stepNodeIds[3]): [¬∀x.Q(x), ¬(∀x.P(x)∧∀x.Q(x)), ∀x.(P(x)∧Q(x)), ¬(...root)]
 * 左枝:
 * 3. ¬∀(pos 0, eigen "a"): [¬P(a), ¬∀x.P(x), ¬(∀x.P(x)∧∀x.Q(x)), ∀x.(P(x)∧Q(x)), ¬(...root)]
 * 4. ∀(pos 3, term "a"): [P(a)∧Q(a), ∀x.(P(x)∧Q(x)), ¬P(a), ¬∀x.P(x), ¬(...∧...), ¬(...root)]
 * 5. ∧(pos 0): [P(a), Q(a), P(a)∧Q(a), ∀x.(P(x)∧Q(x)), ¬P(a), ...]
 * 6. BS: P(a) と ¬P(a)
 * 右枝:
 * 7. ¬∀(pos 0, eigen "a"): [¬Q(a), ¬∀x.Q(x), ¬(∀x.P(x)∧∀x.Q(x)), ∀x.(P(x)∧Q(x)), ¬(...root)]
 * 8. ∀(pos 3, term "a"): [P(a)∧Q(a), ∀x.(P(x)∧Q(x)), ¬Q(a), ¬∀x.Q(x), ¬(...∧...), ¬(...root)]
 * 9. ∧(pos 0): [P(a), Q(a), P(a)∧Q(a), ∀x.(P(x)∧Q(x)), ¬Q(a), ...]
 * 10. BS: Q(a) と ¬Q(a)
 */
