...
const conceptDeductionTheorem: ReferenceEntry = {
  ...
  body: {
    en: [
      ...
      "<b>Example:</b> To prove φ → φ (identity) in a Hilbert system, one can use the Deduction Theorem: assume φ, then φ is immediately derivable, so by the theorem, ⊢ φ → φ. The actual Hilbert-style proof (using S, K, and MP) is considerably longer and less intuitive.",
      "<b>Limitations:</b> The Deduction Theorem does not hold in all logical systems. In particular, for predicate logic, the generalization rule (Gen) requires a side condition: the Deduction Theorem holds only when the hypothesis φ does not contain free occurrences of the variable being generalized.",
    ],
    ja: [
      ...
      "<b>例:</b> ヒルベルト体系でφ → φ（恒等式）を証明するには、演繹定理を使えます: φを仮定すると、φは直ちに導出可能なので、定理により ⊢ φ → φ が得られます。実際のヒルベルト流の証明（S, K, MPを使用）はかなり長く、直観的ではありません。",
      "<b>制限:</b> 演繹定理はすべての論理体系で成り立つわけではありません。特に述語論理では、汎化規則（Gen）に条件が必要です: 演繹定理は、仮説φが汎化される変数の自由出現を含まない場合にのみ成り立ちます。",
    ],
  },
  formalNotation:
