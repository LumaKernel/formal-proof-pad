...
const guideHilbertProofMethod: ReferenceEntry = {
  ...
  body: {
    en: [
      ...
      '<b>The Building Blocks: Axioms and Modus Ponens</b>\nRecall the [[ref:system-lukasiewicz|Łukasiewicz]] axiom schemas:\n• <b>[[ref:axiom-a1|A1]] (K):</b> $\\varphi \\to (\\psi \\to \\varphi)$ — any true formula can be "weakened" by adding an antecedent\n• <b>[[ref:axiom-a2|A2]] (S):</b> ($\\varphi \\to (\\psi \\to \\chi)) \\to ((\\varphi \\to \\psi) \\to (\\varphi \\to \\chi)$) — distributes implication over implication\n• <b>[[ref:axiom-a3|A3]]:</b> ($\\lnot \\varphi \\to \\lnot \\psi) \\to (\\psi \\to \\varphi$) — contraposition: reversing a negated implication\n\n[[ref:rule-mp|Modus Ponens]] (MP) is the sole inference rule: from $\\varphi \\to \\psi$ and $\\varphi$, conclude $\\psi$. The central difficulty in Hilbert proofs lies in choosing the right axiom instances and chaining them with MP.',
    ...
    ja: [
      ...
      "<b>構成要素：公理とModus Ponens</b>\n[[ref:system-lukasiewicz|Łukasiewicz]]公理スキーマは以下の通りである：\n• <b>[[ref:axiom-a1|A1]]（K）：</b> $\\varphi \\to (\\psi \\to \\varphi)$ — 真な論理式に前件を追加して「弱化」できる\n• <b>[[ref:axiom-a2|A2]]（S）：</b> ($\\varphi \\to (\\psi \\to \\chi)) \\to ((\\varphi \\to \\psi) \\to (\\varphi \\to \\chi)$) — 含意を含意の上に分配する\n• <b>[[ref:axiom-a3|A3]]：</b> ($\\lnot \\varphi \\to \\lnot \\psi) \\to (\\psi \\to \\varphi$) — 対偶：否定された含意を反転する\n\n[[ref:rule-mp|Modus Ponens]]（MP）が唯一の推論規則です：$\\varphi \\to \\psi$ と $\\varphi$ から $\\psi$ を導出します。Hilbert系の証明における中心的な課題は、適切な公理インスタンスを選択し、MPで連鎖させることにある。",
...
const axiomA1: ReferenceEntry = {
  ...
  body: {
    en: [
      ...
      "A1 is common to all Hilbert-style axiom systems implemented in this application ([[ref:system-lukasiewicz|Łukasiewicz]], [[ref:system-mendelson|Mendelson]], etc.). It appears frequently in proofs when a previously established result needs to be preserved under additional hypotheses.",
    ],
    ja: [
      ...
      "A1は、本アプリケーションで実装されているすべてのHilbert系公理体系（[[ref:system-lukasiewicz|Łukasiewicz]]、[[ref:system-mendelson|Mendelson]]など）に共通です。以前に確立された結果を追加の仮定のもとで保持する必要がある場合に、証明の中で頻繁に出現します。",
    ],
  },
