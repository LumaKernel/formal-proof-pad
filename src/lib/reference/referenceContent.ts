...
const guideFirstQuestWalkthrough: ReferenceEntry = {
  ...
  body: {
    en: [
      ...
      "<b>The Setup</b>\nOpen quest <quest:prop-01 /> from the quest catalog. The workspace displays a goal: <b>$\\varphi \\to \\varphi$</b>. The <ref:system-lukasiewicz>Łukasiewicz</ref> axiom system provides the following axioms and one inference rule (the palette also shows definition axioms ∧-Def and ∨-Def, but they are not needed for this proof):\n• <b><ref:axiom-a1>A1</ref> (K):</b> $\\varphi \\to (\\psi \\to \\varphi)$\n• <b><ref:axiom-a2>A2</ref> (S):</b> ($\\varphi \\to (\\psi \\to \\chi)) \\to ((\\varphi \\to \\psi) \\to (\\varphi \\to \\chi)$)\n• <b><ref:axiom-a3>A3</ref>:</b> ($\\lnot \\varphi \\to \\lnot \\psi) \\to (\\psi \\to \\varphi$)\n• <b><ref:rule-mp>MP</ref> (Modus Ponens):</b> From $\\varphi \\to \\psi$ and $\\varphi$, derive $\\psi$",
    ...
    ja: [
      ...
      "<b>準備</b>\nクエストカタログから<quest:prop-01 />を開く。ワークスペースにゴール「<b>$\\varphi \\to \\varphi$</b>」が表示される。<ref:system-lukasiewicz>Łukasiewicz</ref>公理体系では以下の公理と1つの推論規則が利用可能である（パレットには定義公理∧-Defと∨-Defも表示されるが、この証明では使わない）：\n• <b><ref:axiom-a1>A1</ref> (K)：</b> $\\varphi \\to (\\psi \\to \\varphi)$\n• <b><ref:axiom-a2>A2</ref> (S)：</b> ($\\varphi \\to (\\psi \\to \\chi)) \\to ((\\varphi \\to \\psi) \\to (\\varphi \\to \\chi)$)\n• <b><ref:axiom-a3>A3</ref>：</b> ($\\lnot \\varphi \\to \\lnot \\psi) \\to (\\psi \\to \\varphi$)\n• <b><ref:rule-mp>MP</ref> (Modus Ponens)：</b> $\\varphi \\to \\psi$ と $\\varphi$ から $\\psi$ を導出",
...
const guideIntroPredicateLogic: ReferenceEntry = {
  ...
  body: {
    ...
    ja: [
      ...
      "<b>命題論理から述語論理のクエストへ</b>\nラベル <quest:pred-01>pred-01</quest> から <quest:pred-06>pred-06</quest> のクエストでは、これらのツールを使って述語論理の証明を構築します。命題論理で学んだパターン — 演繹定理、仮言三段論法、対偶の利用 — は引き続き適用されます。新たな挑戦は量化子と代入を正しく扱うことです。\n典型的な証明は、全称量化された公理をインスタンス化し（A4経由）、その結果を命題推論とMPで操作し、最後にゴールが $\\forall$ を含む場合に一般化（GenとA5経由）するところから始まります。",
