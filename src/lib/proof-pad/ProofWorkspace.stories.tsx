...
/**
 * 整理（Simplification）コンテキストメニュー:
 * α等価な2つのノードを右クリックメニューから接続する完全フロー。
 */
export const SimplificationContextMenu: Story = {
  render: () => {
    const [workspace, setWorkspace] = useState<WorkspaceState>(() => {
      let ws = createEmptyWorkspace(lukasiewiczSystem);
  ...
  play: async ({ canvasElement }) => {
    ...
    // node-1 (all x. P(x)) を右クリック
    const node1 = canvas.getByTestId("proof-node-node-1");
    await userEvent.pointer({ keys: "[MouseRight]", target: node1 });

    // コンテキストメニューに「Connect as Simplification…」があること
    const menuItem = await canvas.findByTestId(
      "workspace-connect-simplification",
    );
    await expect(menuItem).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(
        canvas.queryByTestId("workspace-simplification-banner"),
      ).not.toBeInTheDocument();
    });
  },
};
